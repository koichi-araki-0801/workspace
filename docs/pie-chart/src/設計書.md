...
rev: 1.1 | 2026-06-22 | pie-chart への改称・DB 入力・exe 配布の反映
rev: 2.0 | 2026-07-04 | 章立ての階層化・アーキテクチャ図の追加・初心者向け詳解への拡充
...
pie-chart は TypeScript 製の円グラフ SVG レンダラ。`{name, value}` の配列（JSON / Excel / DB / サンプル）を入力に、ラベルと引出線（leader）を自動配置した円グラフ SVG を生成する。
...
> [!INFO] 長名の 2 行救済は「名前を語中で割らない標準 2 行 `[名前, %]`」（10 章の `twoLineNameFallback`）のみで、**名前を語中で分割する処理は持たない**。
...
重要なのは `forScoring` 引数。**採点（配置候補の比較）時は挙動を固定**してレイアウト選択を baseline と同一に保ち、実描画時のみ `ALWAYS_DRAW_OUTSIDE_LEADERS` などの描画改善を効かせる。「見た目の改善が配置判断を変えて別の退行を生む」ことを防ぐ do-no-harm 設計の中核である。
