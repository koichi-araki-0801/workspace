...
| フォント | `src/svg_export/font.ts` | WOFF2 サブセット埋込（async I/O + キャッシュ。失敗時は full フォントに fallback） | 10 |
...
| エントリ | `src/cli.ts` / `src/test_batch.ts` | CLI（`list` / `one` / `batch`）/ 全 83 サンプル一括生成 + 比較ビューア | 12, 15 |
...
> [!INFO] 長名の 2 行救済は「名前を語中で割らない標準 2 行 `[名前, %]`」（9 章の `applyTwoLineNameFallback`）のみで、**名前を語中で分割する処理は持たない**（旧 `splitLongName` は全廃。`Placement.nameSplit` フラグは後方互換のため型に残置され、現在はどこからも立てない）。
...
## 9.1 overlap 解消（`resolveLabelOverlaps`）— 3 段構成
...
## 9.2 compact cascade（`runCompactCascade`）— 描画前の 1 行化判断
...
## 10.2 SEA（単一 exe）での外部参照

exe 配布（14 章）では、フォント woff2 と `subset-font`（harfbuzz wasm 依存）を **exe に埋め込まず、exe 隣の `fonts/`・`node_modules/` から外部参照**する。バンドルしてしまうと `subset-font` 内部の `require.resolve('harfbuzzjs/hb-subset.wasm')` が解決できず、サブセットが効かずフルフォント埋込に落ちて SVG が肥大化するため。
...
本章の検証（`check` / `batch` / `batch:diff` / `verify` / `verify:consistency` / vitest）は pie-chart 単体のコマンドだが、リポジトリ横断の CI 集約 `pnpm run ci` にも `ci:pie-chart` として組み込まれ、pre-push でローカル実行される。byte-diff の基準となる `out/_baseline` は生成物のためコミット対象外で、意図した挙動変更の際は baseline を更新してから差分ゼロを確認する（更新規律は別冊「pie-chart デプロイ運用手順書」5 章）。
...
フォントの OFL はフォント単体販売の禁止とライセンス文書の同伴を求めるため、`fonts/OFL-BIZUDPGothic.txt` は exe 配布フォルダから外さないこと。新しい依存を足す場合は「端の層に隔離できるか」「決定的出力を壊さないか（11.4 節）」を先に審査する。

# 14. exe 配布（Node SEA + 外部参照）
...
- フォントと `subset-font` を外部参照にする理由は 10.2 節のとおり（サブセット埋込を効かせ、出力を tsx 版と **byte-identical** に保つ）。
...
- do-no-harm の採否述語（better / swapBetter 等）は**パス仕様そのもの** — ヘルパーへ焼き込まず、一字一句変えない（FP 演算順序が変わると数学的等価でも byte が動く。11.4 節）。
...
- **DB 入力の exe 同梱**: `msnodesqlv8` はネイティブ依存のため exe に含めず、利用時に `dist-exe/node_modules` へ追加する運用（14 章）。同梱の自動化は必要が立ってから検討する。
