...
rev: 3.0 | 2026-07-23 | 役割別フォルダ再編（input/layout/svg_export/verify）へ全面追随、Mermaid 4図（状態遷移・アクティビティ・シーケンス・クラス）を追加
rev: 3.1 | 2026-07-24 | 課題・仮説・MVP(1.1)、パフォーマンス(12 章)、依存関係とライセンス(14 章)、リンク集(17 章)の新設と章番号再編
audience: spec
---

本書は pie-chart の構造・レイヤ責務・ラベル配置/引出線アルゴリズム・決定的出力の検証方針を、TypeScript にまだ慣れていない読者でも実装を追えるところまで掘り下げて記述する。一次資料は `pie-chart/README.md` とソースのコメント（コメント規約準拠）。配置パイプラインの設計判断（モード×パス対応・do-no-harm ゲートの使い分け・却下済み設計案）の正典は別冊「pie-chart 設計正典」で、本書はその実装詳解にあたる。本文中のコード抜粋はすべて実ソースからの引用で、引用元をパスで示す。

# 0. 読み進め方（章の依存関係）

はじめて読む場合は 1 → 2 → 3 章の順で全体像をつかむこと。3 章のレイヤ表が本書の目次を兼ねており、以降の章は `renderPdfStylePieToSvg` から呼び出される順（入力 → 設定 → レイアウト → 経路選択 → 描画 → 後処理 → フォント）に並んでいる。

- **改修でラベルの位置がおかしい場合**: 6 章（レイアウト層）→ 7 章（rank カスケード）→ 10 章（emit 修復パス）の順に容疑範囲を絞る。どのモードでどのパスが座標を決めるかの早見表は別冊「pie-chart 設計正典」の配置モード対応表が正典。
- **入力形式を増やす/直す場合**: 4 章（型と入力層）だけで閉じる。
- **出力の見た目（フォント・寸法・配色）を変える場合**: 5 章（設定層）と 11 章（フォント埋込）。
- **変更を入れる前に必ず**: 13 章（決定的 SVG と検証）の byte-diff 手順を読むこと。コメント 1 行の変更でも出力バイト不変の確認が必須である。
- 性能の設計判断は 12 章、依存とライセンスは 14 章、exe 配布は 15 章、CLI・開発規約は 16 章、関連資料へのリンク集は 17 章にまとめる。
...
- 外部依存は端の層に隔離する: Excel 入力のみ `exceljs`（`src/input/load.ts`）、DB(SQL Server) 入力のみ `msnodesqlv8`（`src/input/db.ts`、`optionalDependencies` の遅延 require）、フォント埋込のみ `subset-font`（`src/svg_export/font.ts`）。
- **出力は決定的**（同一入力 → 同一 SVG バイト列）。`out/svg_js/` の SVG 出力は **83 サンプルの回帰テスト**（`src/verify/svg.ts` + `out/_baseline` との byte-diff）で検証する。

## 1.1 課題・仮説・MVP

**課題**: ファンド報告書の円グラフは枚数 × 月次の版数で大量に発生し、ラベルと引出線の配置を人手（DTP）で整えるのは物量的に破綻していた。既製のチャートライブラリは「ラベルが多い・長い・小スライスが密集する」帳票特有の悪条件で重なり・見切れを起こし、結局手直しが要る。

**仮説**: 「配置を『良い形から順に試す』rank カスケードと、修復を『悪化したら戻す』do-no-harm ゲートで組めば、帳票品質のラベル配置を全自動化できる。全自動で収まらない残り数 % は、人が微調整できる別ツール（graph-editor）に委ねる」— 100% の自動化を狙わず、自動 + 手直しの分業として全体を設計している。

**MVP と拡張**: 最初は `samples/` の JSON → SVG 1 枚の変換のみで配置品質を作り込み、その後 Excel / DB（SQL Server）入力と単一 exe 配布（15 章）を足した。**決定的出力**（同一入力 → 同一バイト列）を最初から柱に据えたのは、配置アルゴリズムを改良し続ける前提で「意図しない変化を byte-diff で機械検出する」回帰基盤（13 章）が不可欠だからである。
...
| rank カスケード | ラベル 1 枚の置き方を「良い形」から順に試す仕組み。rank ①（スライス内 2 行原寸）〜⑨（引出線つき外側配置 = 最終手段）まで降格しながら最初に成立した形を採る（7 章）。 |
...
| do-no-harm | 修復パスの採否基準。「適用して不具合数が悪化しないときだけ採用、悪化したら差し戻す（revert）」という原則。計測系は 4 系統あり（10.4 節）、段階で使い分ける。 |
| `EMIT_REPAIR_PASSES` の stage | emit 修復パス列の各エントリが持つ適用先の区分。`'emit'`（既定 = 実出力のみ）/ `'scoring'`（採点のみ）/ `'both'`（両方）。採点列と emit 列を同一テーブルから生成し、列のずれ（drift）を構造的に防ぐ（10.3 節）。 |
...
| Node SEA | Node.js の Single Executable Application。JS バンドルを node.exe に注入して単一 exe を作る仕組み（15 章）。 |
...
| 入力 | `src/input/load.ts` + `samples.json` / `src/input/db.ts` | サンプル読込、JSON / Excel / DB(SQL Server) 入力、`{name, value}` 正規化。DB はネイティブ依存ごと `db.ts` に隔離 | 4 |
| 設定 | `src/config.ts` | 基準寸法・フォント・派生スケール getter 群（`createPieLayoutConfig`）+ 配色（`makeColors`） | 5 |
| 幾何 | `src/layout/geometry.ts` | 純粋幾何/測定/ナッジ系ヘルパー（SVG 文字列生成なし・副作用なし）+ glyph advance 表の束ね | 8 |
| glyph 幅表 | `src/glyph_advance/weight_400.ts` / `weight_700.ts` | 生成物（`npm run gen:widths`）。実フォントの glyph advance 例外表 | 8 |
| レイアウト | `src/layout/diagnostics.ts` | 論理座標でのラベル位置決定（左右割当・Y 解決・flip 判定・X 配置）とモード判定・per-item マーカー付与。公開 API は `layoutLabels` | 6 |
| 経路選択 | `src/layout/placement.ts` | rank ①〜⑨ の配置候補（form / draft）を作るビルダ群と、引出線 SVG path（`leaderPath`） | 7 |
| 描画 | `src/svg_export/rendering.ts` | 論理座標 → SVG px への座標変換、スライス path・テキスト要素（純粋関数） | 9 |
| 引出線幾何 | `src/svg_export/leader_geometry.ts` | 実描画する引出線の点列を確定する `computeDrawnLeader` と交差・貫通・角度整合の計測 | 9 |
| 後処理 | `src/svg_export/post_layout.ts` | overlap 解消（対角押し + 縦分離）・compact cascade・視覚 viewBox nudge・長体クランプ | 10 |
| モード特化 | `src/svg_export/mode_passes.ts` | 配置モード専用パス（左列縦積み / top-band クラスタ / 右上逃がし） | 10 |
| emit 修復 | `src/svg_export/emit_repair.ts` | `EMIT_REPAIR_PASSES` 修復パス列・採点（`countDefects` 系）・do-no-harm ゲート基盤 | 10 |
| フォント | `src/svg_export/font.ts` | WOFF2 サブセット埋込（async I/O + キャッシュ。失敗時は full フォントに fallback） | 11 |
| 統合 | `src/svg_export/pipeline.ts` | `renderPdfStylePieToSvg` の最終アセンブリ、rank カスケード実行エンジン、候補選択 | 3, 7, 10 |
| 検証 | `src/verify/svg.ts` / `consistency.ts` / `oracle_sync.ts` | 独立オラクル検証 / 採点↔emit 一致検証 / オラクル複製定数の drift ガード | 13 |
| エントリ | `src/cli.ts` / `src/test_batch.ts` | CLI（`list` / `one` / `batch`）/ 全 83 サンプル一括生成 + 比較ビューア | 13, 16 |

サイズ感の目安（行数）: `svg_export/emit_repair.ts` 2,305 / `mode_passes.ts` 1,735 / `layout/diagnostics.ts` 1,626 / `svg_export/pipeline.ts` 1,591 / `layout/geometry.ts` 1,153 / `svg_export/leader_geometry.ts` 1,063 / `layout/placement.ts` 968 / `svg_export/post_layout.ts` 721。src 全体で約 14,000 行。
...
各ステップの担当章: 設定生成は 5 章、正規化と `runCompactCascade` は 4 章と 10.2 節、`selectFinalLayout`（`layoutLabels` + 回転 do-no-harm + 右上逃がし枚数探索）は 6 章、`runLabelCascade` は 7 章、`applyEmitRepairPasses` は 10.3 節、`computeDrawnLeader` と Pass 1b〜1e は 9.3 節、フォント埋込は 11 章。
...
- `loadDbItems`（`src/input/db.ts`）: SQL Server へ**単一の SELECT 文のみ**（`assertSelectOnly` が複文・非 SELECT を拒否）を投げ、結果セットの 2 列を読む（`rowsToItems`。列名指定は `nameColumn` / `valueColumn` 両方必須、無指定は先頭 2 列）。`buildConnectionString` が Windows 統合認証（`Trusted_Connection=yes`）の ODBC 接続文字列を組む。ネイティブ依存 `msnodesqlv8` は `createRequire` による**遅延ロード**で、未導入環境でも他入力と `tsc` は動く。

SELECT ガードの実体（`src/input/db.ts`）。CLI から任意 SQL を受け取るため、更新文・複文はここで弾く:
...
# 6. レイアウト層 layout/diagnostics.ts
...
## 6.4 ステップ 2: 混雑診断と配置モード判定（`runDiagnostics` / `deriveModeTags`）
...
上左象限は「finalY（衝突解消済みの位置）」とは別に、描画専用の `upperLeftRenderY` を計算する。自然なスタック位置が天井に収まらないときは、最小詰めスタックと希望位置を線形補間して圧縮する。ここで立つ `useStackRimY` マーカーは、7 章の `buildOutsideRimDraft` が「自然 rim Y の代わりに事前計算の縦スタック値を使う」合図としてレイアウト層と経路選択層をつなぐ。
...
# 7. 経路選択層 layout/placement.ts と rank カスケード
...
## 7.2 ビルダ関数一覧（`src/layout/placement.ts`）
...
**`buildOutsideRimDraft`** — 円外 rim 配置の既定経路。まず特殊経路（`bottomCenterBelow` → 上部「その他」右上 → クラスタ右逃がし → 小スライス右）を順に試し、どれも該当しなければ中心角の外縁に置く。`useStackRimY` が立っているラベルは 6.7 節で事前計算した `upperLeftRenderY` をそのまま使う。
...
引出線は論理座標の点列を M/L 命令に変換して `<path>` に包むだけの純粋関数。点列の中身（どこで曲がるか）は 9 章の `computeDrawnLeader` が決める（`src/layout/placement.ts`）:
...
# 8. 幾何ヘルパー layout/geometry.ts と glyph_advance
...
> [!INFO] 長名の 2 行救済は「名前を語中で割らない標準 2 行 `[名前, %]`」（10 章の `applyTwoLineNameFallback`）のみで、**名前を語中で分割する処理は持たない**（旧 `splitLongName` は全廃。`Placement.nameSplit` フラグは後方互換のため型に残置され、現在はどこからも立てない）。
...
## 9.3 引出線の実描画点列（`computeDrawnLeader`）と leader 磨きパス
...
# 10. 後処理層 post_layout.ts・モード特化 mode_passes.ts・emit 修復 emit_repair.ts
...
## 10.5 モード特化パス（`mode_passes.ts`）

配置モード（6.4 節)ごとの専用パスは `src/svg_export/mode_passes.ts` に集約されている:
...
exe 配布（15 章）では、フォント woff2 と `subset-font`（harfbuzz wasm 依存）を **exe に埋め込まず、exe 隣の `fonts/`・`node_modules/` から外部参照**する。バンドルしてしまうと `subset-font` 内部の `require.resolve('harfbuzzjs/hb-subset.wasm')` が解決できず、サブセットが効かずフルフォント埋込に落ちて SVG が肥大化するため。

- `seaExeDir()` が `node:sea` の `isSea()` で SEA 実行を検出し、exe のあるディレクトリを返す（非 SEA では null）:
...
- **修復は表駆動の 1 周**: `EMIT_REPAIR_PASSES`（10.3 節）はテーブルに列挙された順に 1 回ずつ適用され、do-no-harm ゲートで戻すだけ。フィードバックループを持たないため、パスを足しても計算量は線形にしか増えない。

ラベル数は帳票の性質上たかだか十数枚であり、O(n²) の重なり判定・IoU 的な幾何計算はこの規模では測定誤差の範囲に収まる。上限値を増やす変更は「品質が上がるか」ではなく「83 サンプルの batch 時間と決定性検証の回転が保てるか」で判断すること。

## 12.2 文字幅は静的 glyph advance 表 — 実行時測定ゼロ

ラベル幅の見積りは `npm run gen:widths` が生成する **実 glyph advance の静的表**（`src/glyph_advance/weight_{400,700}.ts`）を引くだけで、`<canvas>` measureText 等の実行時測定を一切行わない。これは性能（測定 API 呼び出しゼロ）と決定性（環境のフォントレンダラに依存しない）と移植性（Node でもブラウザでも同一コード、1 章の柱）を同時に成立させる要の設計である。フォントを差し替えたら表を再生成する運用で、実行時コストは常にハッシュ表参照 1 回に保たれる。

## 12.3 フォントサブセットのキャッシュ（11 章）

WOFF2 サブセット生成（`subset-font`、harfbuzz wasm）は 1 回あたり数十 ms 級で最も重い外部処理のため、`font.ts` の `FONT_FACE_CACHE` が「使用文字集合 + ウェイト」をキーに `@font-face` defs をプロセス内キャッシュする。batch（83 サンプル）で同じ文字集合が繰り返し現れても再サブセット化しない。失敗時は full フォント埋込へフォールバックし、速度より出力の成立を優先する。

## 12.4 決定性 vs 最適化のトレードオフ（最重要の禁止事項）

**「数学的に等価だが浮動小数点の演算順序が変わる」書き換えは、たとえ速くなっても禁止**する。FP 演算は結合順で最下位ビットが動き、座標の丸めが 1 箇所ずれるだけで SVG のバイト列が変わって回帰検証（13 章）の土台が崩れる。do-no-harm の採否述語（better / swapBetter 等）を共通ヘルパーへ焼き込む類のリファクタが禁止なのも同じ理由である（16.3 節）。性能改善はアルゴリズム上限（12.1 節）とキャッシュ（12.3 節）の層で行い、数値計算の書き換えでは行わない。
...
- `npm run batch`（`src/test_batch.ts`）— 全 83 サンプルを `out/svg_js/` に出力し、`out/compare.html`（A4 想定・1 ページ 12 件）を更新。
...
本章の検証（`check` / `batch` / `batch:diff` / `verify` / `verify:consistency` / vitest）は pie-chart 単体のコマンドだが、リポジトリ横断の CI 集約 `pnpm run ci` にも `ci:pie-chart` として組み込まれ、pre-push でローカル実行される。byte-diff の基準となる `out/_baseline` は生成物のためコミット対象外で、意図した挙動変更の際は baseline を更新してから差分ゼロを確認する（更新規律は別冊「pie-chart デプロイ運用手順書」6 章）。

# 14. 依存関係とライセンス

**技術要件**: Node.js（開発は pnpm/Node 24 系、exe 配布物は実行ランタイム同梱で Node 不要）。外部サービス・外部通信は無く、DB 入力時のみ社内 SQL Server へ Windows 統合認証で接続する。

1 章の柱のとおり**レンダリング層は依存ゼロ**で、外部依存は端の層に隔離されている。この構造ゆえに依存表は短い:

| 依存 | 用途（隔離先） | ライセンス |
|---|---|---|
| `exceljs` | Excel 入力のみ（`src/input/load.ts`） | MIT |
| `msnodesqlv8` | DB 入力のみ（`src/input/db.ts`、optional・遅延 require） | Apache-2.0 |
| `subset-font`（+ `harfbuzzjs` wasm） | フォント埋込のみ（`src/svg_export/font.ts`） | MIT（harfbuzz は Old MIT） |
| BIZ UDPGothic（`fonts/` 同梱 WOFF2 400/700） | 出力 SVG への埋込フォント | **SIL OFL 1.1**（`fonts/OFL-BIZUDPGothic.txt` を exe 配布物にも同梱） |
| esbuild / postject（ビルド時のみ） | exe ビルド（15 章） | MIT |
| vitest / tsx / TypeScript（開発時のみ） | テスト・実行・型 | MIT / Apache-2.0 |

フォントの OFL はフォント単体販売の禁止とライセンス文書の同伴を求めるため、`fonts/OFL-BIZUDPGothic.txt` は exe 配布フォルダから外さないこと。新しい依存を足す場合は「端の層に隔離できるか」「決定的出力を壊さないか（12.4 節）」を先に審査する。

# 15. exe 配布（Node SEA + 外部参照）
...
3. node 実行体を `dist-exe/pie-chart.exe` へコピーし、`stripPeSignature` で node.exe 由来の既存 Authenticode 署名を除去してから、**postject** で blob を inject する（既存署名が残ると後段の署名 API が失敗するため）。
...
- 入口は 2 つ: `scripts/build-exe.bat`（→ `build-exe.ps1`。クリーン `npm install` から行う単独環境向け）と `npm run build:exe`（既存 node_modules でビルドのみ行う開発機向け）。詳細・オフライン配布時の同梱物は `pie-chart/README.md`「exe 配布」節を参照。

# 16. CLI・固定パラメータ・開発規約

## 16.1 CLI（`src/cli.ts`）

コマンドは `list` / `one` / `batch` の 3 種。`parseArgs` が `--key value` 形式を解析し、`buildResolveOpts` が入力ソース（`--sample` / `--data-file` / `--data-json` / `--xlsx` / `--sql`）の**排他 1 つ**を検査して `ResolveAsyncOpts` を組み立てる。`buildRenderOverrides` は `--font-weight`（400/700）と `--stroke-ratio`（faux-bold）だけを描画オーバーライドへ通す。`one` の本体は「入力解決 → レンダリング → ファイル書き出し」の 3 行が核:
...
sequenceDiagram
  ...
  DB->>DB: assertSelectOnly<br/>(複文・非 SELECT 拒否)
...
- do-no-harm の採否述語（better / swapBetter 等）は**パス仕様そのもの** — ヘルパーへ焼き込まず、一字一句変えない（FP 演算順序が変わると数学的等価でも byte が動く。12.4 節）。
...
- 別冊: 「pie-chart 操作手順書」（事務担当者向け）・「pie-chart デプロイ運用手順書」（配布・運用者向け）
- `out/compare.html` — `npm run batch` が更新する 83 サンプルの目視比較ビューア
- `samples.json` / `samples/` — 回帰検証の入力サンプル群
- `fonts/OFL-BIZUDPGothic.txt` — 同梱フォントのライセンス原文（配布物にも同梱）
- 関連ツール: graph-editor（本ツールの SVG 出力を人手で微調整する後工程。leader 幾何の端点規約を合わせてある）

## 17.2 既知の改善項目

- **tight-pack 残件**: viewBox はみ出し警告 8 件（13.3 節）。構造的残件であり解消は別タスク。
- **DB 入力の exe 同梱**: `msnodesqlv8` はネイティブ依存のため exe に含めず、利用時に `dist-exe/node_modules` へ追加する運用（15 章）。同梱の自動化は必要が立ってから検討する。
