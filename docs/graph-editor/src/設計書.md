...
6. **Edge をアプリモードで起動**: `msedge.exe --app=<URL> --no-first-run --no-default-browser-check --enable-logging --log-file=data/logs/edge.log --v=1`。**隔離 `--user-data-dir` や `--disable-gpu` は付けない**（VDI クラッシュ対策。12 章）。
...
- `console=False`（黒窓なし）。そのため起動診断は `data/logs/startup.log` が頼り（3 章）。
