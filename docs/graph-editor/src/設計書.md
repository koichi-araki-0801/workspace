...
rev: 1.1 | 2026-06-23 | VDI 対応方針・サンプル集約の反映
rev: 2.0 | 2026-07-04 | 章立ての階層化・アーキテクチャ図の追加・8 モジュール分割の詳細化
rev: 2.1 | 2026-07-05 | 表紙・改訂履歴の新設、経緯記述の整理と文章の推敲
rev: 2.2 | 2026-07-23 | 読み進め方の新設、4 種の図（活動・順序・状態遷移・クラス）追加、テスト実行コマンドを package.json 経由へ
rev: 2.3 | 2026-07-24 | 課題・仮説・MVP(1.1)、パフォーマンスと応答性(9 章)、依存関係とライセンス(11 章)、リンク集(14 章)の新設と章番号再編
audience: spec
---
...
**読み進め方**: 全体像だけ知りたいときは 1〜3 章（概要・用語・アーキテクチャ）まで。挙動を追う
ときは 4 章（サーバ `app.py`）→ 5 章（初期化・イベント・状態遷移）→ 6 章（編集モデル）の順が実行
順と一致する。保存形式は 8 章、性能・応答性の設計は 9 章、改修時の検証は 10 章、依存とライセンス
は 11 章、配布は 12 章。VDI 由来の設計制約（本ツール最大の「なぜ」）は 13 章にまとまっており、
関連資料へのリンク集は 14 章。簡潔な設計正典は別冊「graph-editor 設計正典」を参照。
...
**pie-chart との関係**: pie-chart はレイアウトを全自動で決めるが、まれに人の目でしか判断できない微調整（この 1 枚だけラベルを少し上げたい等）が残る。graph-editor はその**後工程の手直し専用ツール**であり、次の分担になっている。

```
[入力データ] → pie-chart（自動配置・SVG 生成） → *.svg → graph-editor（人手で微調整） → 修正版 *.svg
```

- **pie-chart のレンダラ（`pie-chart/src/`）には一切依存しない**。出来上がった SVG を後段で手直しする道具。
- ブラウザエンジンを同梱せず OS の Edge を使うため、配布物は**単一 exe（~10MB）**。
- 元データは書き換えない。あくまで生成済み SVG の見た目を調整する。

## 1.1 課題・仮説・MVP

**課題**: pie-chart の自動配置は大半のチャートで完結するが、まれに「この 1 枚だけラベルを少し動かしたい」という人の目でしか判断できない微調整が残る。この残り数 % のために元データや配置アルゴリズムを触るのは割に合わず、かといって汎用ドローツール（Inkscape 等）は事務担当者には過剰で、教育・導入コストが高い。

**仮説**: 「ラベルと引出線を動かす、だけの単機能エディタなら、OS 標準の Edge をアプリ窓に使う構成で単一 exe ~10MB に収まり、インストール不要で VDI を含む社内全端末に配れる」。フォントが SVG に base64 で自己内包されている（pie-chart 側の出力仕様）ため、閲覧・編集にフォント環境を要求しない点がこの構成を成立させている。

**MVP**: 「開く → ドラッグ → 保存」の 3 段ウィザードのみで、それ以上の機能（テキスト編集・要素追加・配色変更）は意図的に持たない。用途を pie-chart の後工程の微調整に限定することで、UI も検証も小さく保っている。
...
| File System Access API | ブラウザからネイティブの「開く/保存」ダイアログを出す新しめの API（`showOpenFilePicker` 等）。本ツールは **VDI で不安定なため使わない**（13 章）。 |
...
| `resources/web/js/` | 計 8 本 | エディタの全ロジック（3.2 節） |
| `resources/web/lib/leader_geom.cjs` | 62 | 引出線まわりの DOM 非依存な純粋関数（UMD、7 章） |
| `requirements.txt` | — | 実行は標準ライブラリのみ。`pyinstaller` はビルド時のみ |
| `scripts/build.bat` / `build.ps1` | — | exe をワンクリックでビルド（`--onefile`、12 章） |

## 3.2 フロントエンドの 8 モジュール（`resources/web/js/`）
...
## 3.3 編集フロー全体（アクティビティ図）

SVG を開いてから保存するまでの処理の流れ。各ノードの実装は 5〜8 章で順に詳述する。
...
1. **ロギング初期化**（`_setup_logging`）: `data/logs/startup.log` へ INFO 出力。exe は `console=False` で標準エラーが見えないため、ファイル出力で起動診断を可観測にする。
...
6. **Edge をアプリモードで起動**: `msedge.exe --app=<URL> --no-first-run --no-default-browser-check --enable-logging --log-file=data/logs/edge.log --v=1`。**隔離 `--user-data-dir` や `--disable-gpu` は付けない**（VDI クラッシュ対策。13 章）。
...
- **ハンドルのドラッグ**: ハンドルの `pointerdown` → `startLeaderDrag`。このとき `_auto = false` を立て、以後この引出線は位置駆動ルール（5.3 節）で上書きしない（手動編集の尊重）。
...
## 5.3 状態モデル（`label-state.js`）と位置駆動ルール
...
- ドラッグ中は `onDrag` が `pointermove` / `pointerup` / `pointercancel` を window に配線し、
  **実際に動いた最初の 1 回だけ** `pushHistory` する（5.2 節）。
- ハンドルドラッグ開始時に `_auto = false` が立ち、以後その leader は位置駆動ルールの対象外に
  なる（5.3 節の leader 状態モデル）。
...
- 文字は `<text>` の `transform=translate` で動かし、**保存時に `x` / `y` へ焼き込んで transform を除去**する（8 章）。
...
**保存先はダウンロードフォルダ固定**。`utils.js` の `hasFsAccess()` は**常に false を返す**設計（VDI の管理された Edge ではネイティブピッカーがレンダラごとクラッシュするため。13 章）で、保存は常に `Blob` + `<a download>` によるダウンロードになる。保存先を選ぶダイアログは出ない。元ファイルは上書きされない。

# 9. パフォーマンスと応答性

編集ツールの品質は「ドラッグに引っかかりが無いこと」で決まる。本章はその応答性を支える 3 つの仕組みと、意図的な割り切りをまとめる。

## 9.1 描画は rAF スケジューラに合流（5.2 節）
...
## 9.3 起動時の `fonts.ready` 待ちと規模の割り切り
...
- サーバ側は静的配信のみで性能要素は無い。idle watchdog（60 秒）はゾンビプロセス防止であり、応答性には関与しない（4.1 節）。
...
- **単体テストできるのは `leader_geom.cjs` を DOM 非依存の純粋関数に切り出してあるから**（7 章）。幾何ロジックをブラウザ実装から分離したのは、テスト可能性そのものを狙った構造であり、ブラウザと vitest が同一実装を共有する。
...
フォントは同梱しない — 編集対象の SVG が pie-chart 側で base64 自己内包しているため（1.1 節）、本ツール側にフォントのライセンス管理は発生しない。外部通信も無い（127.0.0.1 のみ）。
...
- `console=False`（黒窓なし）。そのため起動診断は `data/logs/startup.log` が頼り（4 章）。
- 診断ログは exe 隣の `data/logs/`（`startup.log` = Python 側起動診断、`edge.log` = Edge 側ログ）に出る。ポータブル配置（exe をどこに置いてもその隣に生成）。

# 13. VDI / リモートデスクトップ対応

VDI（仮想デスクトップ）環境の「管理された Edge」で安定動作させるため、次の 3 点を設計原則とする。

1. **Edge は特殊フラグ無しで起動する**。隔離 `--user-data-dir` + `--disable-gpu`（ソフトウェア描画固定）のような非標準構成は、VDI では Edge のレンダラが不安定になり**アプリ窓がクラッシュ**するため使わない（`edge.log` に `GetGpuDriverOverlayInfo failed ...` が出る。通常の Edge ブラウジング自体は安定）。**安定動作している既定 Edge と同じ構成**で開く。
...
3. **サーバ寿命を Edge プロセスに縛らない**。Edge のコールド起動はプロセスを再起動するため、プロセス監視での終了判定は誤爆する。終了は `/quit` ビーコン + アイドル watchdog（60 秒）に一本化する（4.1 節）。

> [!INFO] 落ちる・開かない場合は `data/logs/startup.log`（Python 側）と `data/logs/edge.log`（Edge 側）で要因を切り分ける。
...
- 関連ツール: pie-chart（前工程の SVG 生成器）。`leader_geom.cjs` は pie-chart の `leader_geometry.ts` と**独立の並行実装**だが、端点の BBox クランプ規約を合わせてある — leader の幾何仕様を変えるときは pie-chart 詳細設計書 9 章と突き合わせること（7 章 WARN）。

## 14.2 既知の改善項目

- **編集対象の拡張はしない方針**: テキスト編集・要素追加・配色変更は意図的な非機能（1.1 節）。要望が立った場合も、まず pie-chart 側（自動配置の改良）で解決できないかを先に検討する。
- **保存先がダウンロードフォルダ固定**: VDI 安定性との交換条件（13 章）。File System Access API の復活はしない（却下済み設計）。
