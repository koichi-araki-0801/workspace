...
rev:
  ...
  - 2.3 | 2026-07-24 | 課題・仮説・MVP(1.1)、パフォーマンスと応答性(8 章)、依存関係とライセンス(10 章)、リンク集(13 章)の新設と章番号再編
  - 2.4 | 2026-08-15 | フロントエンドを 13 モジュール構成へ更新、行数の再集計、記述の整理
audience: spec
---
...
| ファイル | 行数 | 役割 |
|---|---|---|
| `app.py` | 1030 | 標準ライブラリの小さな HTTP サーバ（127.0.0.1）で `resources/web/` を配信し、Edge をアプリモードで起動・常駐管理。同一オリジン検査・セッショントークン・資源上限もここ。実行時の外部依存なし |
...
| `resources/web/js/` | 計 13 本 | エディタの全ロジック（2.2 節） |
| `resources/web/lib/leader_geom.cjs` | 67 | 引出線まわりの DOM 非依存な純粋関数（UMD、6 章） |
| `requirements.txt` | — | 実行は標準ライブラリのみ。`pyinstaller` はビルド時のみ |
| `scripts/build.bat` / `build.ps1` | — | exe をワンクリックでビルド（`--onefile`、11 章） |

## 2.2 フロントエンドの 13 モジュール（`resources/web/js/`）

フロントエンドのロジックは責務別の 13 モジュールに分割されている。`editor.js` の `Editor` クラスがセッション状態とライフサイクルを持ち、入出力（`editor-io.js`）・描画（`editor-render.js`）・イベント配線（`editor-events.js`）は `Editor` インスタンスを第 1 引数に取る関数群として分離されている。

| モジュール | 行数 | 責務 |
|---|---|---|
| `main.js` | 66 | エントリポイント。`Editor` 生成 → イベント配線 → 画面 1 表示。`window.__editor` の公開、`/quit` ビーコン、`/ping` ハートビート |
| `editor.js` | 479 | エディタ本体。セッション状態（開いたファイル群・選択・履歴）+ ライフサイクル + 描画統括を担う `Editor` クラス |
| `editor-io.js` | 361 | SVG 読込（`load` / `abortLoad`）・保存（`save` / `bakeSvg`）・ファイル選択と D&D。入出力の関所 |
| `editor-render.js` | 334 | オーバーレイ（ハンドル）・インスペクタ（右パネル）・左レールの描画。状態を持たず `Editor` の状態を読み書きする |
| `editor-events.js` | 110 | 静的 UI 要素（ドロップゾーン / ツールバー / ステップバー / キーボード）への起動時イベント配線 |
| `label-state.js` | 197 | 1 ラベル分の状態モデル `LabelState` と実 DOM 同期（`renderToDom`） |
| `pie-rules.js` | 64 | 円幾何と leader 既定配置の純粋関数（`parsePieGeometry` / `labelBox` / `isOutsidePie` / `computeDefaultLeaderPts`）。DOM 非依存で vitest が単体検証する |
| `svg-policy.js` | 341 | 未信頼 SVG の許可リスト（要素 / 属性 / 値文法）。`sanitizeSvg` が参照する述語のみで DOM 非依存 |
| `constants.js` | 86 | 調整用定数（`CONFIG.condenseMin` 等）・既定スタイル・UI 文言・操作表の一元管理 |
| `dom.js` | 45 | UI 要素の id 参照を一元化する DOM ハンドル集 |
| `geom.js` | 10 | `leader_geom.cjs`（グローバル `LeaderGeom`）を ES モジュールへ橋渡しして再エクスポート |
| `icons.js` | 51 | アイコンの inline SVG 化とトースト通知 |
| `utils.js` | 217 | 状態を持たない純粋ユーティリティ（`sanitizeSvg` / `hasFsAccess` / `STATE_FIELDS` 等） |
...
1. **ロギング初期化**（`_setup_logging`）: `startup.log` へ INFO 出力。exe は `console=False` で標準エラーが見えないため、ファイル出力で起動診断を可観測にする。置き場は配布 exe では `%LOCALAPPDATA%\LabelEditor\data\logs`（可変状態をプログラムフォルダの外へ出す。pdf-to-svg と同じ判断）。明示指定は `LABELEDITOR_DATA_DIR`、ユーザー領域が使えない端末（VDI）だけ exe 隣の `data\logs` へ落ちる。ソース実行はリポジトリ直下 `data/logs`。
...
SVG ファイルを開いてから編集可能になるまで（`editor-io.js` の `openFiles` → `load`）:
...
選択中ラベルにはオーバーレイとして丸ハンドルが出る。色と役割は `buildOverlay`（`editor-render.js`）で決まる:
...
セグメントトグルで「引出線 表示/非表示」「曲げ点 あり/なし」「文字色 黒/白」「行数 1 行/2 行」を切り替え、**長体スライダ**で名前部分だけを横圧縮する（`editor-render.js` の `INSPECTOR_BODY_HTML`）:
...
DOM 非依存の純粋関数集（67 行、UMD）。ブラウザでは classic script としてグローバル `LeaderGeom` になり、Node では `module.exports` で読める。プロジェクトの `package.json` が `"type": "module"` のため拡張子は `.cjs`。
...
| `test/editor_pie_rules.test.ts` | vitest 単体 | `pie-rules.js` の円幾何・leader 既定配置の純粋関数 |
| `test/editor_svg_policy.test.ts` | vitest 単体 | `svg-policy.js` の許可リストが迂回入力を拒否すること |
| `test/editor_state_fields.test.ts` | vitest 単体 | `utils.js` の `STATE_FIELDS` / `stateEquals`（編集済み判定） |
...
| `test/editor_ops.e2e.ts` | Playwright E2E | Undo/Redo・保存 bake のクリーン性・インスペクタの実クリック配線 |
| `test/editor_sanitize.e2e.ts` / `editor_load_guards.e2e.ts` | Playwright E2E | 未信頼 SVG の迂回入力が実行されないこと・読込時の資源上限と中断時の空状態 |
| `test/capture_docs.e2e.ts` | Playwright | 操作手順書向けスクリーンショット取得（`docs/graph-editor/images/` へ出力） |
| `test/test_*.py` | pytest | `app.py` の同一オリジン検査・トークン認可・資源上限・データ置き場・Edge 起動引数・pdf-to-svg との並行実装 drift |
...
- **単体テストできるのは `leader_geom.cjs` / `pie-rules.js` / `svg-policy.js` を DOM 非依存の純粋関数に切り出してあるから**（6 章）。幾何・許可リストのロジックをブラウザ実装から分離したのは、テスト可能性そのものを狙った構造であり、ブラウザと vitest が同一実装を共有する。
