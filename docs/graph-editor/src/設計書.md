...
- 診断ログは exe 隣の `data/logs/startup.log`（Python 側起動診断）に出る。ポータブル配置（exe をどこに置いてもその隣に生成）。Edge 側の verbose ログは**既定で出さない**（既定プロファイルで開くため、利用者の Edge セッション全体の診断が配布物の隣に溜まり、回覧時に一緒に渡ってしまうのを避ける）。必要なときだけ `LABELEDITOR_EDGE_LOG=1` を立て、`%LOCALAPPDATA%\LabelEditor\logs\edge.log` を見る。
...
> [!INFO] 落ちる・開かない場合はまず `data/logs/startup.log`（Python 側）を見る。Edge 側の詳細が要るときだけ `LABELEDITOR_EDGE_LOG=1` を立てて再現させ、`%LOCALAPPDATA%\LabelEditor\logs\edge.log` を見る（既定では出力されない）。
