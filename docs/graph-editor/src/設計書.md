...
かつて `ui.html` に集約されていたロジックは、現在は責務別の 8 モジュールに分割されている。
...
実行コマンド: `pnpm exec vitest run`（単体）/ `pnpm exec playwright test`（E2E）/ `pnpm run typecheck`。
...
1. **Edge は特殊フラグ無しで起動する**。当初は隔離 `--user-data-dir` + `--disable-gpu`（ソフトウェア描画固定）で起動していたが、VDI ではこの非標準構成だと Edge のレンダラが不安定で**アプリ窓がクラッシュ**した（`edge.log` に `GetGpuDriverOverlayInfo failed ...`。通常の Edge ブラウジング自体は安定）。そこで**安定動作している既定 Edge と同じ構成**で開く方針に変更した。
