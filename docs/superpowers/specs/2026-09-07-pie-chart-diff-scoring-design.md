...
表の上限値は、下で述べるゲート由来の割合を丸めた比率から求めている。n=12 の 70.8% は
「残り約 15%」を採った 0.85 × 5/6 で、正確な比率 14.11% を使うと 71.6%(達成度 90.5%)になる。
n=14 の 59.1% も同様に 0.69 × 6/7 の値である。加えて、上限の計算に使う「採点 1 回あたりの
対数」は leader を全件描く場合の値なので、leader を描かない配置があるぶん実効の上限は
さらに下がる(丸めとは別の、未定量の下方補正)。いずれにせよ達成度は約 9 割である。
