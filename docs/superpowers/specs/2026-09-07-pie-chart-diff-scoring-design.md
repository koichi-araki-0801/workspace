...
| ケース | `pairTests` 基準値 | 実測値 | 削減率 | 理論値 |
|---|---|---|---|---|
| `gen_long_12_other` | 実装時に記入 | 実装時に記入 | 実装時に記入 | 1 回あたり 6 分の 1 |
| `gen_long_14_other` | 実装時に記入 | 実装時に記入 | 実装時に記入 | 1 回あたり 7 分の 1 |

理論値の根拠: 全走査の採点 1 回は cross `n(n-1)/2` + through `n(n-1)` = `1.5·n(n-1)` 対。
差分(`changed` = 1 index)は cross `(n-1)` + through `2(n-1)` = `3(n-1)` 対。比は `n/2`。
回数の主因である S1 / S2 がこの比で減り、全走査のまま残す S7 / S8 / S11 は 1 チャートあたり
十数回なので、全体の削減率はこの比に近づく。

**実測が理論値を大きく下回る場合は実装の取りこぼし**(フォールバックが意図せず効いている、
差分化したはずのサイトが全走査を呼んでいる等)とみなし、原因を特定してから受入する。
