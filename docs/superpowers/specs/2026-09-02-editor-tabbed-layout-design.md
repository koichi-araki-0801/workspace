...
```
[📄 RET  Report Edit Tool]      [編集][テンプレート作成][承認]│[結合PDF][比較]│[履歴]            [管理者][🌙][user ▾]
```

- 現行の 2 段（`h-14` ヘッダ + `border-t` のタブ `nav`）を 1 つの `header` にまとめる。
  ロゴ（アイコン + `RET` + 副文言「Report Edit Tool」。**現行のまま何も落とさない**）→
  タブ群 → 右端（管理者 / テーマ / ユーザー）の順。高さは 56px 固定。
- ゾーン間の間隔は十分にあける: ロゴとタブ群の間は `gap-8`（32px）以上、タブ群と右端は
  `ml-auto` で残り幅すべてを空ける。タブ同士の間隔（`gap-1`）とグループ区切り（`mx-1` の
  縦線）は現行を保つ。
- タブの見た目（`RouterLink` の class・グループ区切り・承認待ち件数バッジ）は現行を保つ。
- 幅の概算は約 1205px（ロゴ 174 + タブ 672 + 右 271 + ゾーン間 48 + 左右 padding 40）。
  1760px 枠には収まる。ビューポート 1245px 未満で溢れる場合は `header` 全体を
  `overflow-x-auto` にする（現行の `nav` と同じ作法。要素を隠さない）。
