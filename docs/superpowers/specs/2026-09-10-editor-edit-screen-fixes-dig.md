...
| ④ fetch の言語 | **Python `offline/fetch_offline_bundle.py` + ASCII `.bat`**。stdlib（`urllib`/`hashlib`）のみ。`.sha256` 検証は pytest で固定 | Python 第一方針。fetch 側端末は Python 3.13 前提で足りる | `.sha256` 検証が `verify.ps1` と二重化する（一致検査の置き方は計画で決める） |
...
| コミット分割 | **6 コミット**: ④ Python fetch / 保存形式（幾何 inline 化 + 自動 id 除去 + protectedCss）/ dirty 機構（③⑥）/ ① 起動 100% + スクショ再撮影 / ⑤ UI 状態 / ② kind 撤去。editor-data の汚染修正は別リポで 1 commit | 保存形式の変更を単独で bisect できる | 保存形式のコミット直後は「選択だけで未確定」が残る（次コミットで解消） |
...
4. ④ の fetch は Python 第一方針（メモリ `team-python-first`）に従い Python で書く（発見 C）。
...
| Q6 | fetch の言語 | Python + ASCII `.bat`、stdlib のみ、pytest で固定 | Python 第一 | 低 | `.sha256` 検証が `verify.ps1` と二重化 |
...
- **fetch を .ps1 で書く**: Python 第一方針に反する。
...
- **`.sha256` 検証の二重実装**（Python fetch と `verify.ps1`）: 同じ穴が両側に入る形は一致検査では
  検出できない（設計正典「並行実装の drift 検査」の教訓）。pytest では「検証失敗・取得失敗のどちら
  でも配置先を汚さない / 一時ディレクトリを残さない / 3 つの URL」を固定する。
...
1. **④ Python fetch**（`offline/fetch_offline_bundle.py` + ASCII `.bat`。`setup-offline.ps1` から
   取得経路（`-Owner/-Repo/-Tag`・`Download-File`）を撤去し、直下にも `bk\` にも無ければ fetch を
   案内して停止。bk\ 退避は「直下のバンドルを使った回」）
   - テスト: pytest（downloader を差し替え、検証失敗・取得失敗のどちらでも配置先を汚さない /
     一時ディレクトリを残さない / 3 つの URL）。`test:docs` 相当の pytest 対象に `offline/` を足す
     （`package.json` の `ci` 集約に組込む）。`scripts/check-comments.py` の対象リストに新規 `.py` を
     加える。Pester（`verify.Tests.ps1`）は既存のまま。
