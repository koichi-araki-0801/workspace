...
PartNote>`）と判定し、投稿 1 件へ変換して返す。変換後の `id` は `` `legacy:<pathKey>` ``
とする（固定値 `legacy` 単体だと、`entryId` の所在特定はファイル内の全 `pathKey` を横断して
行うため、1 ファイルが複数パーツを持つ旧形式では全パーツが同じ ID を名乗ってしまい、編集・
削除が別パーツへ誤爆する）。`pathKey` を連結すればファイル内で一意になる（旧形式は 1 パーツに
つき 1 件しか持てないため）。ランダム ID を都度振ると読むたびに編集・削除の宛先が変わって
しまう。`createdAt` / `createdBy` には旧 `updatedAt` / `updatedBy` を充てる。
...
- `MAX_NOTE_ENTRIES_PER_PART = 200` を新設する。1 パーツあたりの投稿数上限。これが無いと
  ...
  `validation` で拒否し、既存投稿の編集と削除は上限に関わらず必ず通す。この 200 件は
  ファイル（= 版インスタンス）単位の上限であり、表示は交付版⇄全体版をマージした 1 本の
  スレッドなので、1 パーツの表示件数は両ファイル分を合わせて最大 400 件になりうる。
...
- **右ペイン（`Inspector.vue`）**: メモセクションは**追加専用**にする。件数バッジ・textarea・
  「追加」ボタン・説明文（「既存のメモはキャンバスの吹き出しに表示します。交付版と全体版で
...
- **左右どちらへ出すかは、吹き出しが帳票の上に重なる量が少ない側で決める。**
  `gapRight = ページ右端 − パーツ右端` と `gapLeft = パーツ左端 − ページ左端` を比べ、
  `gapRight <= gapLeft` なら右、そうでなければ左。ページ幅いっぱいのパーツは両者 0 で同値に
  なるので既定の右へ出る（実運用のパーツはほぼこれに当たる。メモの単位は `.page` 直下の
  top-level ブロックで、ページ幅いっぱいのことが多い）。
- **吹き出しは常にページの上へ重ねる**（表計算ソフトのセルコメントと同じ挙動）。選んだ側の
  パーツ内側へ寄せて置く（右なら吹き出しの右端をパーツ右端に、左なら吹き出しの左端をパーツ
  左端に合わせる）。
- 縦はパーツ上端に合わせ、canvas の上下からはみ出す分はクランプする。
- 吹き出しは常に 1 つだけ開く（選択パーツのもの）。閉じるボタンで隠し、再度選択で開く。

> **改訂（2026-09-02）**: 当初は「吹き出しと反対側へページを寄せて場所を作り、寄せても
> 幅が足りないときだけ重ねる」設計だったが、実機確認で**原理的に成立しない**と判明し撤回した。
...
> （中央寄せの margin を 0 にしても動くのはその半分の ~31px）。一方で場所の判定は
> `getElementPos` が返す**視覚幅**（倍率で縮んだ後の幅）で行っていたため、判定は「余白は
> 十分」と誤って結論し、実際には吹き出しがページへ重なって描画される乖離が生じていた。
> ページの大きさ・倍率を変えずに余白を作る方法は無いので、常に重ねる方式へ単純化した
> （ユーザー判断）。上のリーダー線の記述・下の「ページを寄せる」に関する記述は当初設計の
> 名残であり、現行はリーダー線を持たず、ページの水平位置も動かさない。

**ページ（帳票 HTML 本体）の描画サイズは変えない。** 吹き出しは非スクロールの overlay 層に
重ねるだけで、ページの側は一切動かさない。具体的には次をしない。

- 吹き出しの幅を確保するためにズーム倍率を下げる（`useZoomFit` のフィット計算に吹き出しの
  幅を含めない）。
- canvas の描画領域を狭めて（右に固定幅の欄を作って）ページを押し縮める。
- ページの `transform` / 余白 / スケールを吹き出しの都合で変える。
- ページの水平位置を吹き出しの都合で動かす（上記改訂により撤回済み）。

理由は、ページの見えは PDF の見えと一致していることが判断の前提であり、メモという注釈の
都合で本体の見えを変えてはならないため。加えてページ寸法・位置が動くと、既存の overlay
（選択枠・ハンドル・ページ境界ガイド・メモ目印）の座標計算がすべて連動して揺れる。
...
- 吹き出しの配置（左右の判定・縦クランプ）は `useCanvasMarkers` の純粋な位置計算として
  切り出し、単体テストを置く（`noteBubbleLayout.test.ts`）。
...
- 吹き出しの置き場を作るためのキャンバス側のレイアウト変更（ズーム調整・描画領域の縮小・
  ページの縮小）。前節のとおり本体の見えは動かさない。
