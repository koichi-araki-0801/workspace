...
### `Inspector.vue`（メモセクション）

- 投稿リストを作成日時の昇順で表示する。1 件は「名前 ・ 日時 ・ 版種名」の見出し行と本文
  （改行を保持）、および編集・削除の操作から成る。版種名は `templateId` を
  `parseTemplateFileName` で解いて得る（保存はしない）。
- 編集はインラインの textarea と「保存」「取消」。
- 追加はリスト下部の textarea と「追加」ボタン。
- 削除は共通の確認ダイアログ（`components/ui/confirm.ts` の `confirm()`）を経る。画面独自の
  確認 UI は作らない。
- セクションの説明文を実際の挙動に合わせる:「このパーツへのメモを、交付版と全体版で共有します
  （基準日ごとに独立）」。
- バッジは「記入あり」から件数表示へ変える。
