...
- **左右どちらへ出すかは、リーダー線が帳票の上を横切る量が少ない側で決める。**
...
- **吹き出しを出す側と反対へページを寄せて場所を作る。** 右へ出すならページを左寄せ、左へ
  出すならページを右寄せにする。寄せるのは吹き出しを開いている間だけで、閉じたら中央へ戻す。
  **寄せるのは位置だけで、ページの大きさは変えない**（次項）。
- 縦はパーツ上端に合わせ、canvas の上下からはみ出す分はクランプする（クランプで縦がずれた
  ときのリーダーは L 字にする）。
- **寄せても余白が吹き出しの幅に足りないときはページの上に重ねる**（ズームを上げるとページが
  キャンバス幅を超え、寄せても場所が作れない。表計算ソフトのコメントと同じ挙動で、余白の
  有無に依存しない）。
...
前項の左寄せ・右寄せは**キャンバス内でのページの位置だけ**を動かす（横方向のスクロール位置に
相当する移動）。寸法・倍率には触れない。

理由は 2 つ。ページの見えは PDF の見えと一致していることが判断の前提であり、メモという注釈の
都合で本体の見えを変えてはならない。加えてページ寸法が動くと、既存の overlay（選択枠・
ハンドル・ページ境界ガイド・メモ目印）の座標計算がすべて連動して揺れる。余白が足りない場合の
答えは「ページを縮める」ではなく「吹き出しをページに重ねる」で統一する。
