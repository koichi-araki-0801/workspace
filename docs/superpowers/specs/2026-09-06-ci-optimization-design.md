...
- 計画 B2 の実測(2026-09-07): perFile 有効化後の `test:coverage` 全体通しは 208.30 秒(exit 0、221
  ファイル・2943 テスト全緑)。include 追加: `routes/*.ts`・`rest/*.ts`(`reviewRepo.ts`/`http.ts`
  の個別列挙を置換)・`repositories.ts`。本番コード変更は 4 点: `previewHost.bundleSafeToInline`
  export・`inlineDocScripts` の定数 export・`egressGuard` の probe 注入と中継先ポート解決の
  1 箇所への統合・`nunjucksRender` の到達不能分岐除去。
