...
### 追加で修正した 2 件（ユーザー判断により同一セッションで実施）

Phase 3〜5 で見つかった次の 2 件も、検証後に修正した。

#### 発見 4: 却下理由がサーバで必須になっていない → `7ffda0b` で修正
...
- **影響**: API を直接叩けば理由を残さず差し戻せる。権限昇格ではないが、「なぜ差し戻されたか」が
  監査ログに残らない経路がある。
- **修正**: 却下だけに専用スキーマ `ReviewRejectBody`（`comment` を trim 後 1 文字以上）を与えた。
  承認・保留のメモは任意のまま。local モードの Repository 実装にも同じ検査を入れ、同じ操作が
  local では通り rest では 400 になる食い違いを作らない。画面側は元から理由必須だったため
  UI の変更は不要。`openapi.json` は生成物なので `openapi:gen` で再生成した。
- **テスト**: 空ボディ・空文字・空白のみの 3 形が 400 で弾かれ、申請が `pending` のまま残ること。
  あわせて「承認はメモ無しでも通る」ことも固定した（必須の射程を却下理由だけに留める）。

#### 発見 5: docs のビルドとテストが `python` を直接呼ぶ → `a681c40` で修正
...
- **経緯**: python-tools 側はリポジトリ分離の際に同じ修正を済ませており（`py -3.13` へ統一）、
  monorepo だけが取り残されていた。相互反映のルールに載っていない箇所だった。
- **修正**: `build_all.bat` の 3 箇所と `package.json` の `test:docs` を `py -3.13` へ。
  CLAUDE.md の手順記述も直し、「Python の起動は `py -3.13` に統一する」を明記した。

##### 修正中に踏んだ事故: `.bat` を編集すると改行が LF になる

`build_all.bat` を編集したところ、cmd が行を分割できなくなり、`rem` 行が命令として
解釈されるようになった（`'同梱の' is not recognized`）。さらに `rem` 行に含まれていた半角の
矢印がリダイレクトとして働き、リポジトリ直下に `閲覧用` という空ファイルが作られた。

原因は**編集ツールが LF で保存したこと**の 1 点。CRLF へ戻すと、同じ内容のままエラーが完全に
消え、空ファイルも作られなくなった。切り分けとして「CRLF の bat で `rem ... -> ...` を実行する」
最小ケースを作り、ファイルが作られないことを確認している。**`.bat` を編集したら必ず CRLF を
保つこと**。なお本番の実害は無い（コミットされていたのは CRLF 版で、`.gitattributes` が
チェックアウト時に CRLF へ戻す）。

### 発見 6: `capture_docs` が描画の完了を待たずに撮る → `fbcafb3` / `90462e8` で修正

- **症状**: `capture_docs.spec.ts` が撮った `docs/editor/images/history-tab.png` が、履歴テーブルの
  スケルトン表示（読み込み中のプレースホルダ）のまま、タブも遷移途中（「比較」が選択された状態）で
  固定されていた。`editor-handles.png` も内容が欠けた状態だった。
- **気づいた経緯**: 再撮影の差分を「いつもの追随」として一度コミットしてしまい、次の CI で
  同じ画像が 75%（history-tab）・27%（editor-handles）も大きくなったことで発覚した。
  **バイトサイズの大きな変動は、レンダリングの微差ではなく内容の欠落を疑う手がかりになる**。
- **影響**: 手引き HTML は画像を base64 で埋め込むため、読み込み中の画面がそのまま
  ドキュメントの成果物に載る。壊れていることが CI では検出されない（テストは通る）。
- **修正**: 2 段構え。
  - 一覧を持つ画面（編集・作成・比較・履歴・管理者・承認キュー）は、撮影前に
    スケルトンの消滅を待つ（`waitForLoaded`）。`animate-pulse` を持つのは `Skeleton.vue`
    だけなので、0 件であることを実データの描画完了と見なせる。
  - ハンドル撮影（`editor-handles.png`）は可視化待ちでは足りなかった。canvas のフォントが
    載る前は行の高さが確定せず、その寸法で `clip` を取ると内容が途中で切れる。**連続 2 回
    同じ寸法**になるまで待ってから撮る（`waitForStableBox`）。
- **効果の実測**: `history-tab.png` の再撮影差分が **75% → +1 バイト**へ収束。
  `editor-handles.png` は CI の並列実行下でも欠けた版（13,228 bytes）に戻らず 16,818 bytes
  を維持した。**この不具合は CI の並列実行下でしか再現せず、単独実行では出ない**
  （最初の修正が単独実行では通ったのに CI で戻ったことで判った）。
- **残る性質**: 再撮影のたびに 1% 未満のバイト差は出る（PNG 圧縮の揺れ）。内容の欠落とは
  桁が違うので、**差分の大きさで判別できる**。1% 未満は破棄してよい。
