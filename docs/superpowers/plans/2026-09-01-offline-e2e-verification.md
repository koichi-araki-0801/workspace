...
### 未修正の発見: `capture_docs` が描画の完了を待たずに撮ることがある
...
- **修正案**: 撮影前に「スケルトンが消えたこと」を待つ（テーブル行の実データ、または
  読み込み完了を示す要素の出現を待機条件にする）。現状は `page.waitForTimeout` 相当の
  時間待ちか、遷移直後の撮影になっている可能性が高いので、spec の待機条件を見直す。
- **暫定対応**: 正しく撮れた版へ差し替えた（`915ec82`）。
