...
rev:
  ...
  - 2.6 | 2026-09-03 | 承認タブ再構成・保留撤去・コメント機能
audience: spec
---
...
| `reviews` | 承認タブ（対象テンプレートの申請を縦に並べて決着） |
...
2. **一覧**: 承認タブ（`ReviewTabView`）。対象は編集タブで開いているテンプレート 1 件（`?template=` → `tabMemory` の直前画面）で、そのテンプレートの申請を状態の要約箱（承認待ち / 承認済み / 却下）で絞り、新しい順のアコーディオンに並べる。approver/admin は全件、editor は自分の申請のみ見える。展開した区画だけが `ReviewDetail` を持ち、同時展開は 2 件まで。各区画の右にコメントパネルを置く（コメントの仕組みは 8.3 節末尾の `NoteRepository` の説明を参照）。
3. **精査**: `ReviewDetail` は 2 つの表示タブを持つ。既定タブ「見た目で比較」（`ReviewVisualCompare`）は修正前｜修正後を `PreviewPanel`（7.1 節の隔離プレビューホスト）2 面で左右に並べ、実際の帳票と同じ組版で見せる（ページ送りは左右連動、変更のあったページには黄色マーカー、`create` 申請は単面）。もう 1 つのタブ「文字の変更を一覧で見る」は従来どおりパーツ単位（`.page` 直下ブロック）で「変更前｜変更後」を並べる。diff 計算はどちらのタブでも版比較と同じ `htmlWorker.buildHtmlDiff` を共有し、`edit` 申請の比較元は現行公開版、`create` 申請は空（全パーツ追加扱い）とする。画面上部の `ReviewNoticeBar` は「書式設定変更」「印刷専用 CSS」「一覧の表示しきれない項目」「一覧打ち切り」の 4 種の技術的注意点を「⚠ 画面だけでは確認しきれない変更が N 件」の 1 行へ集約し、展開すると業務語での説明と、印刷専用 CSS のときは修正後 PDF を新規タブで開く導線を出す。カード上の `changedSummary`（8.1 節）はここでは再表示せず、実差分（`summary`/`rows`）だけを表示する。
...
5. **却下**: `reviewRepo.rejectReview` も `assertUndecided` を経由し、`pending` から呼べる。実ファイル非更新。理由（`decision.comment`）必須で、申請者に表示される。
