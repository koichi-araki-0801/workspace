...
rev:
  ...
  - 2.4 | 2026-08-15 | 実装との突合による記述の訂正（CSP・Jinja 描画の隔離・sproc 7 本・別冊名）と文章の推敲
  - 2.5 | 2026-08-27 | 精査画面改修の反映（held 状態・見た目比較タブ・通知バー集約を 8 章へ追記）
audience: spec
---
...
| `../../editor-data/`（リポジトリ外） | テンプレート実体の data リポジトリ（既定 `dataRoot`）。`templates/`（.html）・`css/`（ファンド毎）・`drafts/`（自動保存。git 管理外）・`reviews/`（承認申請。git 管理外）を持ち、確定保存ごとに 1 コミットで版管理する |
...
- `toTemplate(editable)`: 逆変換。Jinja トークンを一旦 Unicode 私用領域（PUA。通常の文字と衝突しない予約コード）の placeholder に退避してから（必要なら js-beautify で整形し）最後に文字列置換で復元する。これにより HTML シリアライザが `<` `>` `&` をエスケープして Jinja を壊す事故を防ぐ。
...
flowchart TD
  ...
  B --> C["placeholder 退避<br/>PUA（私用領域）の文字へ"]
  C --> D{"pretty?"}
...
（生 Jinja HTML へ直接 `formatHtml` をかけるのは厳禁。`{% for %}` 等を壊すためである。CSS は静的前提で `formatCss` を直接使える。）
...
- **子フレームは認証付きサブリソースを要求しない**（設計正典の不変則）: opaque オリジンからの要求には SameSite=Lax のセッション cookie が付かないため、テンプレート JS・フォントは `web/src/lib/previewSelfContain.ts` が postMessage 直前に文書へインライン展開する（親 = 認証済み・同一オリジンが取得）。
...
**結合 PDF（`POST /api/build/merge`）**: 結合PDF タブ（`features/merge/`）が複数テンプレートを選択順に値埋めし、レンダ済み文書の配列を送る。サーバ側では `mergeInput.ts` が一時ディレクトリへ `doc-000.html` …と `entry` 配列の `vivliostyle.config.cjs` を実体化し、既存 `buildProjectPdf` に委譲する。複数 entry は vivliostyle が book（単一 spine）として組版するため `page` カウンタが文書間で継続し、各文書へ付加する共通 CSS（`MERGE_PAGE_COUNTER_CSS`）が通しページ番号を印字する。テンプレート CSS 中の `counter-reset/set: page` は結合経路限定で除去する（`stripPageCounterReset`）。draft は適用せず、結合対象は台帳の登録済み（確定）内容のみとする。上限は次のとおり。
...
`ReviewStatus` = `pending | approved | rejected | held`。`ReviewOrigin` = `edit | create`（どちらの系統からの申請かを保持）。申請本体（`ReviewRequest`）は html / css / filledHtml を含み、一覧用の `ReviewRequestMeta` と分離している。
...
stateDiagram-v2
  ...
  pending --> held: holdReview
  held --> approved: approveReview
  held --> rejected: rejectReview
  held --> held: holdReview(コメント更新)
  approved --> [*]
  rejected --> draft: 修正して再申請
```

`draft` は自動保存の作業コピーで、置き場は `dataRoot/drafts/`（git 管理外）。`ReviewStatus` には含まれない。却下後の再申請は新しい申請 id で `pending` を作り直す。`held` は「精査者が判断を保留した」状態で、`pending` と同じく精査者が操作可能（`ReviewDiffView` の `canDecide` は `pending || held` を許す）。`holdReview(reqId, comment?)` は `pending`/`held` の双方から呼べ、`held` からの再呼び出しはコメント（保留メモ）の上書きとして扱う（自己遷移）。`heldBy`/`heldAt`/`holdComment` は `ReviewRequestMeta` に保持し、一覧カードの保留メモ表示に使う。`changedSummary`（申請時に申請者側が自己申告する変更箇所数・名前）は監査・一覧の**参考表示専用**で、精査画面の実差分計算（8.2 節）とは独立している。

## 8.2 データフロー

1. **申請**: プレビュー画面の「確定保存を申請」→ `PreviewView.submitForReview`（`origin = route.query.created === '1' ? 'create' : 'edit'`。ここでも 2 系統を判定する）→ server `reviewRepo.submitReview` が `status: 'pending'` と `baseHash`（現行公開版の sha1）を記録して `dataRoot/reviews/` に保存する。**実ファイルは変更しない**。
2. **一覧**: 承認キュー（`ReviewQueueView`）。approver/admin は全件、editor は自分の申請のみ見える。
3. **精査**: `ReviewDiffView` は 2 つの表示タブを持つ。既定タブ「見た目で比較」（`ReviewVisualCompare`）は修正前｜修正後を `PreviewPanel`（7.1 節の隔離プレビューホスト）2 面で左右に並べ、実際の帳票と同じ組版で見せる（ページ送りは左右連動、変更のあったページには黄色マーカー、`create` 申請は単面）。もう 1 つのタブ「文字の変更を一覧で見る」は従来どおりパーツ単位（`.page` 直下ブロック）で「変更前｜変更後」を並べる。diff 計算はどちらのタブでも版比較と同じ `htmlWorker.buildHtmlDiff` を共有し、`edit` 申請の比較元は現行公開版、`create` 申請は空（全パーツ追加扱い）とする。画面上部の `ReviewNoticeBar` は「書式設定変更」「印刷専用 CSS」「一覧の表示しきれない項目」「一覧打ち切り」の 4 種の技術的注意点を「⚠ 画面だけでは確認しきれない変更が N 件」の 1 行へ集約し、展開すると業務語での説明と、印刷専用 CSS のときは修正後 PDF を新規タブで開く導線を出す。カード上の `changedSummary`（8.1 節）はここでは再表示せず、実差分（`summary`/`rows`）だけを表示する。
4. **承認**（approver/admin のみ）: `reviewRepo.approveReview` の要点は次の 4 つ。`pending`/`held` 以外は 409 / **自己承認拒否**（admin 除く）/ `staleWarning`（申請後に公開版が変わっていたら警告。ブロックはしない）/ `applyConfirmedSave`（実ファイル書込 + git 1 コミット）:

```ts
// assertUndecided: approved/rejected(決着済み)だけを 409 で拒む。pending/held は通す。
...
5. **却下**: `reviewRepo.rejectReview` も `assertUndecided` を経由し、`pending`/`held` の双方から呼べる。実ファイル非更新。理由（`decision.comment`）必須で、申請者に表示される。
6. **保留**: `reviewRepo.holdReview`（`pending`/`held` から可）は実ファイル非更新のまま `status: 'held'` と `heldBy`/`heldAt`/`holdComment` を記録し、精査キューへ戻す。承認・却下と異なり決着を付けない中間状態で、精査者は一覧の「保留中」から `holdReview` 呼び出し前の状態に戻ることなくそのまま承認・却下へ進める。
...
> [!INFO] 関連する `NoteRepository` はパーツ単位のコメントを追記型スレッドとして扱う。投稿は構造パスキー（`pathKey`）でパーツに紐づき、読み書きとも版インスタンス（`templateId`）単位に閉じる（交付版⇄全体版のペアでも共有せず、基準日をまたぐ繰り越しもしない）。各投稿は `status`（open / resolved）・`replyTo`（親投稿 id。null なら親）・`kind`（note / fix-request / question）を持ち、返信は同じパーツの親投稿にだけ付く（返信への返信は不可）。状態の切替は親投稿にだけ許し、返信へ伝播する。親投稿の削除は返信を道連れにする。編集画面ではキャンバス上の吹き出し（`NoteBubble.vue` / `noteBubbleLayout.ts`）に選択パーツの既存スレッドを表示し、右ペインは追加専用にする。承認画面の行ラベルと編集画面のパーツ語彙（`partKey.ts`）を共有する基盤でもある点は変わらない。
...
7. `@scalar/fastify-api-reference` で `/api/docs`（API リファレンス UI。オフライン動作可）。
   ...
   **経路別 CSP 一覧**（encapsulate された register が `onSend` で専用ヘッダを当てる。全域を緩めない）:
   ...
   | 経路 | script-src | 理由 |
   |---|---|---|
   | 全域（helmet 既定 + `config.buildCspDirectives`） | `'self'` + index.html のハッシュ | SPA 本体。`'unsafe-inline'` も `'unsafe-eval'` も入れない（作成タブの値差込 `toFilled` は `lib/jinjaExpr.ts` の許可リスト評価器で、`new Function` を使わない） |
...
> [!INFO] 確定テンプレートを直接書き換える REST 経路は持たない。保存経路は必ず申請（`POST /api/review-requests`）→ 承認（`POST /api/review-requests/:reqId/approve`）を通り、実ファイルへ書けるのは `applyConfirmedSave` だけである（8 章）。
...
- `ddl/`（テーブル・索引・制約）→ `sproc/`（7 本: `audit` / `noteMaster` / `part` / `sample` / `session` / `template` / `user`）→ `seed/`（管理ユーザー = `hash-password.ts` 生成物、パーツカタログ・サンプルデータ = `gen-seed.ts` 生成）を `apply.ps1`（`sqlcmd -E -f 65001`）で順適用する。
...
`start.bat` は Node 24 の確認・初回 `pnpm install`・ポート 24680/24681 の事前チェック（自リポジトリ由来の stale プロセスのみ自動停止）・run-in-job（Job Object）によるプロセスツリーの後始末まで代行する。CLI からはルートで `pnpm dev`（= dev/local 相当）でもよい。
...
運用機への配置・起動・LAN 公開・証明書・ファイアウォールの実手順は別冊「Editor 配布・運用手順書」を正典とする（本書では重複させない）。構成の設計意図は 9.3 節（LAN/HTTPS）と 13.2 節（起動モード）を参照。
