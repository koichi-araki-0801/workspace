...
> [!INFO] 関連する `NoteRepository` はパーツ単位のメモを追記型スレッドとして扱う。投稿は構造パスキー（`pathKey`）でパーツに紐づき、読み取り時に交付版⇄全体版（ペア版種）をマージして 1 本のスレッドとして返す（基準日をまたぐ繰り越しはしない）。編集画面ではキャンバス上の吹き出し（`NoteBubble.vue` / `noteBubbleLayout.ts`）に選択パーツの既存メモを表示し、右ペインは追加専用にする。承認画面の行ラベルと編集画面のパーツ語彙（`partKey.ts`）を共有する基盤でもある。
