...
- `MainLayout` の子: `edit`（`/` 既定）/ `create` / `compare` / `reviews`・`reviews/:reqId` / `history` / `merge` / `admin`（`meta.admin`）。
...
| `merge` | 結合PDF タブ（複数テンプレ → 通しページ番号付き 1 PDF。7.2 節） |
| `admin` | ユーザー管理 |
...
- `web/src/lib/nunjucksRender.ts`: 生 Jinja2 + サンプルデータを Nunjucks（`autoescape: true`）で近似描画し、sanitize（DOMPurify。**`<script>` は残す** — テンプレ JS は正当なコンテンツ）・外部 `<link>` 除去・CSS インライン化を経てプレビュー用ドキュメントを組み立てる。プレビュー専用で確定ファイルには影響しない。
- 組版は**アプリ本体の document ではなく、opaque オリジンの隔離 iframe の中**で行う（2026-08-06）。`PreviewView.vue` → `PreviewPanel.vue` が `sandbox="allow-scripts"`（`allow-same-origin` なし）の iframe にサーバ配信のビューアホストページ（`server/src/vivliostyle/previewHost.ts` が `/api/preview-host/index.html` を経路専用 CSP で配信。`@vivliostyle/core` バンドル内蔵）を載せ、文書・ページ送り・ズームは postMessage 契約（`shared/src/preview/hostProtocol.ts`）で運ぶ。テンプレ JS はこの iframe 内で動き、アプリのオリジン・cookie・API には届かない。
- **子フレームは認証付きサブリソースを要求しない**（設計正典の不変則）: opaque オリジンからの要求には SameSite=Lax のセッション cookie が付かないため、テンプレ JS・フォントは `web/src/lib/previewSelfContain.ts` が postMessage 直前に文書へインライン展開する（親 = 認証済み・同一オリジンが取得）。
...
**結合 PDF（`POST /api/build/merge`）**: 結合PDF タブ（`features/merge/`）が複数テンプレを選択順に値埋めし、レンダ済み文書の配列を送る。サーバは `mergeInput.ts` が一時ディレクトリへ `doc-000.html` …と `entry` 配列の `vivliostyle.config.cjs` を実体化し、既存 `buildProjectPdf` に委譲する。複数 entry は vivliostyle が book（単一 spine）として組版するため `page` カウンタが文書間で継続し、各文書へ付加する共通 CSS（`MERGE_PAGE_COUNTER_CSS`）が通しページ番号を印字する。テンプレ CSS 中の `counter-reset/set: page` は結合経路限定で除去（`stripPageCounterReset`）。本文サイズ上限はルート単位 `bodyLimit`（`vivliostyle.build.maxMergeBytes`、既定 32MB、env `VIVLIO_MAX_MERGE_BYTES`）、文書数上限は 30（`BuildMergeRequest`）。draft は適用しない — 結合対象は台帳の登録済み（確定）内容のみ。
...
7. `@scalar/fastify-api-reference` で `/api/docs`（API リファレンス UI。オフライン動作可）。
   ...
   **経路別 CSP 一覧**（encapsulate された register が `onSend` で専用ヘッダを当てる。全域を緩めない）:
   ...
   | 経路 | script-src | 理由 |
   |---|---|---|
   | 全域（helmet 既定 + `config.buildCspDirectives`） | `'self' 'unsafe-eval'` + index.html のハッシュ | SPA 本体。`'unsafe-inline'` は入れない |
   | `/api/docs`（`openapi/docsRoutes.ts`） | Scalar の inline script を許す専用 CSP | nonce 無し inline を吐くため encapsulate |
   | `/api/preview-host/**`（`vivliostyle/previewHost.ts`） | `'self' 'unsafe-inline' 'unsafe-eval'` | テンプレ inline JS は可変文字列でハッシュ不能。射程は opaque iframe 内のみ |
...
- **初回パスワード変更の強制**: `要パスワード変更` が立っているセッションは、サーバ側 `requireAuth` が `POST /api/auth/init-password`・`GET /api/auth/me`・`POST /api/auth/logout` の 3 経路以外を 403 で拒否する。SPA の authGuard による `/password-init` への誘導（5.1 節）は UX であって施錠ではない。変更には**現在のパスワード**の提示が必須で、忘れた場合の復旧は管理者リセットのみ（未認証の自己再設定経路は持たない）。
- **KDF とレート制限**: PBKDF2（SHA-512・12 万回）は非同期 API のみを使い、イベントループを塞がない。ログイン試行は `auth/loginRateLimit.ts` が (IP, ログインID) 単位で 5 分窓 5 回失敗 → 15 分拒否とし、KDF より手前で締める。ハッシュは `server/scripts/hash-password.ts` で生成して seed に投入。
