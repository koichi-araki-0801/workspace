...
`draft` は自動保存の作業コピーで、置き場は `dataRoot/drafts/`（git 管理外）。`ReviewStatus` には含まれない。却下後の再申請は新しい申請 id で `pending` を作り直す。
...
9. `config.requireAuth` が真なら `invalidateAllSessions()` を呼び、**サーバ再起動で全セッション失効**とする（11 章）。
