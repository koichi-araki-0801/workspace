...
> [!INFO] **DB の守備範囲**: DB は台帳（作成可能カタログ）・パーツ・認証・監査のみで、**テンプレート本体はファイル + git が正**。`listTemplates` は `filled/`（値入り HTML）+ `pending/`（生成直後の未確定）のファイル走査、`getTemplate` は `filled/` → `templates/` → `pending/` の順に読み、いずれも sproc を通らない。「本体=ファイル/git、DB=台帳」が `rest` の基本方針。
...
7 契約と永続先の対応（`rest`。route 対応の一覧表は 9.2 節）。「本体=ファイル/git、DB=台帳」（2.1 節）が形になっている:
...
`local`（fixtures + localStorage）と `rest`（REST + SQL Server）は**同じ Repository 契約**を満たすため、束ねオブジェクトの差し替えだけで切り替わる。画面コードは無改修:
...
`filled` の値は `resolveFilled`（`web/src/api/local/store.ts`）が決める。編集タブの承認で上書きした値入り HTML → 未編集 fixture の静的 filled の順で引き、作成タブの承認で Jinja が変わった id（`htmlOverride` が在る）は静的 filled が古いので空扱いにし、editor に再差込させる:
...
- **再起動失効**: rest モードは起動フックの `invalidateAllSessions`（session sproc `'全失効'`）。local モードは `web/src/lib/appEpoch.ts` が「配信側の起動 epoch（SPA shell に注入）とログイン時 epoch の食い違い」を検知して旧セッションを破棄する。これにより両モードで「サーバ/dev 再起動 = 全員再ログイン」の挙動が等価になる。再ログイン画面には再起動バナーを表示する。
...
- **DB**（`rest` のみ）: SQL Server 2012 + ODBC Driver 17（Windows 統合認証、10.3 節）。`local` は DB 不要。
