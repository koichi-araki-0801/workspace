...
| Fastify | Node.js の Web サーバフレームワーク。server パッケージの土台（旧記述の Express は誤り。10 章）。 |
| sproc ゲートウェイ | すべての DB アクセスを「第 1 引数 `@操作` で分岐する 1 本のストアドプロシージャ」に集約する方式（11 章）。 |
...
主要なドメイン型は `shared/src/schemas.ts` の **Zod スキーマが正典**で、`shared/src/index.ts` は `z.infer` で TS 型を導出する（手書き interface との二重定義は廃止済み。手書きで残るのは `FundResolution` / `ConfirmSaveRequest` / `TemplateInstance` のみ）。スキーマは実行時検証（server の `validate.ts`）と OpenAPI 生成（zod-openapi）にも共用される:
...
ハイライト CSS は `jinjaComponents.ts`。**素の `.jinja-chip.jinja-var` に背景を直書きせず**、body に `jinja-vars-highlight` クラスが付いたときだけ効かせる（直書きすると編集タブへハイライトが漏れる過去退行 `aa9bd65` の再発になる）:
...
4. **承認**（approver/admin のみ）: `reviewRepo.approveReview` の要点は 4 つ——pending 以外は 409 / **自己承認拒否**（admin 除く）/ `staleWarning`（申請後に公開版が変わっていたら警告。ブロックはしない）/ `applyConfirmedSave`（実ファイル書込 + git 1 コミット）:
...
> [!WARN] 旧版の本書はサーバを Express と記述していたが、実装は **Fastify** である（`fastify` / `@fastify/cookie` / `@fastify/helmet` / `@fastify/static`）。
...
> [!INFO] `PUT /api/templates/:id`（直接保存）は承認ゲート導入後も **requireApprover 限定の緊急用**として残している（設計メモの「撤去」とは異なる現状）。通常の保存経路は必ず申請 → 承認を通る。

# 11. DB アクセス層

## 11.1 sproc ゲートウェイ方式

ターゲットは **SQL Server 2012**、DB `usrap`、スキーマ `ug01`、テーブル接頭辞 `Rep1_運報自動化_Editor_`。すべての DB アクセスは「第 1 引数 `@操作` で分岐する 1 本のストアドプロシージャ」を通し、物理名（日本語）は `sprocNames.ts` の 1 箇所に集約する:
...
- **再起動失効**: REST モードは起動フックの `invalidateAllSessions`（session sproc `'全失効'`）。ローカルモードは `web/src/lib/appEpoch.ts` が「配信側の起動 epoch（SPA shell に注入）とログイン時 epoch の食い違い」を検知して旧セッションを破棄する——両モードで「サーバ/dev 再起動 = 全員再ログイン」の挙動が等価になる。再ログイン画面には再起動バナーを表示する。
