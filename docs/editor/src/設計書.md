...
> [!INFO] **DB の守備範囲**: DB は台帳（作成可能カタログ）・パーツ・認証・監査のみで、**テンプレート本体はファイル + git が正**。`listTemplates` は `filled/`（値入り HTML）+ `pending/`（生成直後の未確定）のファイル走査、`getTemplate` は `filled/` → `templates/` → `pending/` の順に読み、いずれも sproc を通らない。「本体=ファイル/git、DB=台帳」がフェーズ2 の基本方針。
...
切替は `web/src/main.ts` の 2 行（環境変数 `VITE_API_MODE`）。画面へは Vue の provide/inject（`useRepos()`）で配られる。既定は rest。`local` は明示指定のときだけ:
...
getTemplate: (id: string) =>
  attempt(() => {
    ...
    // 編集タブの承認で上書きした値入り HTML → 未編集 fixture の静的 filled の順。
...
`getTemplate` は `filled/`（値入り HTML）→ `templates/`（作成タブの Jinja）→ `pending/` の順に探し、
`filled/` で見つかれば `html` と `filled` の両方に本文を返す。
...
編集タブが読む「値埋め込み済み HTML」の出どころは local / rest で異なる。

local は `genFilled.ts`（`pnpm --filter web run gen:filled`）が `fixtures/templates/*.html`（生 Jinja）
+ `fixtures/sample/<fund>.json`（**per-fund 実値**）から `fixtures/filled/*.html`
（= `Template.filled`）を事前生成する:
...
rest は dataRoot の `filled/<テンプレID>.html` を返す。別ツールが置き、承認（`origin='edit'`）が
上書きする。値入り HTML はテキストノードがすでに実値へ差し込み済みで、属性内 Jinja は
（`toFilled` 由来のものと同様に）round-trip 保持のため設計上残りうる。プレビュー・比較・
...
Windows では `editor/start.bat`（ダブルクリック可）で起動する。引数は順不同で、ビルドモード（`dev`/`prod`・既定 prod）× データモード（`rest`/`local`・既定 rest。`db` は `rest` の別名）× `lan` を組み合わせる:
...
`start.bat` は Node 24 の確認・初回 `pnpm install`・ポート 24680/24681 の事前チェック（自リポジトリ由来の stale プロセスのみ自動停止）・run-in-job（Job Object）によるプロセスツリーの後始末まで代行する。CLI からはルートで `pnpm dev`（= dev/rest 相当）でもよい。
