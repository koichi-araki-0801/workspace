...
- ポートは **server=24680 / Vite dev=24681** で固定。起動は `editor/start.bat`（引数順不同:
  `dev`/`prod` × `rest`（既定）/`local` × `lan`）。`rest` は SQL Server + 認証必須、`rest lan` は
...
| `web/src/api/repositories.ts` | rest（既定）⇄ local（fixtures+localStorage、開発用）の**唯一の差し替え点**（`VITE_API_MODE`） |
...
- **編集 2 系統**（根幹。詳細は設計書 6.1 節）: 編集タブ = `tpl.filled`（per-fund 実値）+
  ...
  経路判定は `route.query.created === '1'` のみ。rest の `tpl.filled` は dataRoot
  `filled/<id>.html`（別ツールが置く値入り HTML。テキストノードは実値差込済み、属性内 Jinja は
  round-trip 保持のため残りうる）。承認は申請の `origin` で `filled/`（edit）と `templates/`
  （create）を書き分け、値入り HTML はプレビュー・比較・結合 PDF でも nunjucks を通さない。
  `status:'draft'`（pending だけの id）は作成経路（`?created=1`）で開く。`created` query を
  出す規則は `features/templates/editorRoute.ts` の `editorRoute`、pending の判定は同ファイルの
  `opensAsCreate` で、一覧・承認タブ・プレビューの戻る導線が共用する。編集経路（`origin='edit'`）
  の申請・承認は `filled/<id>.html` の存在を要求する（server `assertFilledPresentForEdit`、
  local `assertEditSubmissionAllowed`）。後者は内部で `resolveFilled` を引くほか、
  本文が空の申請も拒否する（空文字を「filled 無し」の印に使うための local 限定の規則で、
  server 側に対応は無い）。local の値入り HTML の有無は `resolveFilled`（`filledOverride` →
  作成承認済みなら無し → fixture）で決める。作成承認後に旧 `filled/` が残る rest とは意図的に
  違う（local は別ツールの配置運用を持たない）。
...
- **パーツコメントは 1 段の入れ子スレッドで、版インスタンスごとに独立する**: コメントは
  ...
  (open / resolved)・`replyTo`(親投稿 id。null なら親)を持つ(種別は持たない。コメントは
  メモ 1 種類。旧データ・旧クライアントの `kind` は読み取りで捨てる)。返信は同じパーツの
  親投稿にだけ付く(返信への返信は不可)。状態の切替は親にだけ許し、返信へ伝播する。親の削除は
  返信を道連れにする。上限は「ファイル 4MB」「`pathKey` 1000 件」「1 パーツ 200 投稿(返信を
  含む)」の 3 段で、件数上限だけではサイズを守れない(1 パーツでファイル上限に達すると、
  そのテンプレの全コメントが保存不能になる)。旧形式(1 パーツ 1 件)と 2 属性を持たない投稿は
  読み取り時に既定値(open / null)を補って読む(ID は `legacy:<pathKey>` 固定)。
- **コメントの一覧は右ペイン、スレッドの表示はキャンバスの吹き出し**: 右ペインは
  「プロパティ | コメント」の切替を持ち、コメント側(`features/editor/comments/CommentPanel.vue`)
  はテンプレート全体のコメントを一覧にする(検索 = 本文・投稿者 / 絞り込み = 状態・
  ...
  `selectPartByKey` で選択して canvas をスクロールする。吹き出しは選択しただけでは開かず、
  マーカーのクリック・一覧の行クリック・投稿の追加で開く(選択が変われば閉じる)。開いたときは
  吹き出しは**常にページへ重ねて出す**(表計算ソフトのセルコメントと同じ挙動)。左右どちらへ
  出すかは、吹き出しが帳票の上に重なる量が少ない側で選ぶ。**ページの大きさ・倍率は変えない**
  — ページの見えは PDF の見えと一致している必要があり、注釈の都合で本体を縮めない。配置計算は
...
- **編集キャンバスの赤入れ表示は生 DOM の装飾で、モデルには載せない**: 確定版の値埋め込み本文
  ...
  表示は既定 OFF。ON にすると行送り・改ページが PDF とずれる（仕様）。
...
- **保存内容に GrapesJS 由来の揮発物を載せない**: 幾何（幅・余白）は inline `style` 属性に
  ...
  保存 CSS へ混入して load のたび積み増すのを止める）。この変更のスパイク検証で確認したのは
  table の auto layout が `box-sizing` の差を吸収した 1 テンプレのみで、flex/grid +
  `width:%` + padding/border で寸法を出すテンプレは canvas 上の見えが変わりうる（実画面での
  確認が必要。box-sizing が要るなら per-fund CSS 側で宣言する）。`avoidInlineStyle` は GrapesJS
  0.23 系で deprecated 扱いで、false にしても `getCss()`（`CssGenerator.buildFromModel`）は
  inline style を持つ component へ無条件で `#<自動id>{…}` をミラーする（対応する `id` 属性は
  HTML 側に出ないため必ず死んだセレクタになる） — `buildFromModel` はこの設定値を生成のたびに
  読むだけなので、`getCss()` は `editor.getConfig().avoidInlineStyle` を呼び出しの間だけ
  `true` に立てて `ed.getCss()` を呼び、`finally` で元へ戻す（ミラーを生成元で止める。
  component 木の走査や文字列除去をこちら側で再実装しない — GrapesJS の内部形式・述語と
  二重実装すると版が上がるたびズレうる）。同期呼び出しの間だけの切替なので他経路
  （`patchSelectedStyle` 等）には影響しない。既定 `forceClass:true` は component 生成時に
  inline style が非空だと自動生成クラスへ丸ごと
  移し替えるため、`forceClass:false` も併せて必要（無いと load の再パースで幾何が
  inline style → 自動クラスへ化けて round-trip が壊れる）。Undo ミラーは `:v2`（旧ミラーは
  読まない）。機械検証は `web/test/saveFormat.dom.test.ts`。
...
- **`allowEdit` を永続する**: しない。編集許可はリロード後に安全側の既定 OFF から始める必要が
  ある（誤って未確認のテンプレへ編集可能な状態で着地させない）。プレビュー往復（同一セッション）
  でだけ `sess.ui` 越しに値が残るのは、それがリロードを跨がない in-memory の状態だから。
...
- **自動 id を保存内容に載せる**: しない。選択で GrapesJS が作る自動 id（`i[a-z0-9]+`）は
  `getBodyHtml()`/`getCss()` の両方から落とす。draft / Undo snapshot に混入すると再読込で
  確定版と構造キーが一致しなくなる。
- **protectedCss を保存 CSS に載せる**: しない。`protectedCss:''` で canvas の下地を
  `a4CanvasCss` 側に一本化し、`getCss()` が load のたび `box-sizing`/`body{margin}` を
  積み増さないようにする。
...
- **コメントのあるパーツを選んだだけで吹き出しを開く**: しない（Q16）。紙面へ重なる吹き出しが
  頼んでいないのに出る形になる。開くのはマーカーのクリック・一覧の行クリック・投稿の追加の
  明示操作でだけ、選択が変われば閉じる。
...
- **全テンプレート横断のコメント検索 API**: 足さない。検索は開いているテンプレート 1 件の中で
  クライアント側に閉じる。
- **ペア同期の状態ファイルを `filled/` と `templates/` で分ける**: しない。両方にあるのは
  作成承認直後の短期間で、混在は競合→スキップの fail-safe。分けると JSON 形式変更と移行が要る。
- **編集タブのバナーに `templates/` 側の競合を出す**: しない。バナーは値入り HTML のペアの
  有無を見る。`templates/` 側はその版種自身の承認時に扱う。
...
3. カバレッジは **include 列挙 = テスト済みのみ・ファイル単位で全指標 85% 閾値**。新規ファイルを
   テストしたら include へ追加し、そのファイル単体で 85% を満たすこと（正典は `editor/README.md`）。
4. CI 集約は `pnpm ci`（`check:comments → check:claude-hooks → check:ci → test:scripts →
   ...
   コミットする。両 project とも `editor/server/scripts/e2e-rest-server.ts` のサーバ（sproc は
   in-memory フェイク `server/test/fakes/sprocFake.ts`、dataRoot は一時ディレクトリを毎回作り直し）
   を相手に走り、SQL Server は不要。フルの `ci` は `pie-chart:batch` → `pie-chart:batch:diff` で
   `pie-chart/out/_baseline` を要求するため、clone 直後はまずルート `README.md` の
   「フル `ci` の前提」節に従って基準を 1 回作る。
