...
- **⚠ 未完了の記録**: プレビュー（vivliostyle）の opaque オリジン iframe 化は**未実施**。
  現状は script 除去 + `previewProxy.CONTENT_CSP` の `script-src 'none'` のままで、
  **JS が動くのは PDF と承認画面の 2 つだけ**。プレビューと PDF で見た目が食い違う。
  `@vivliostyle/core` は `window` を受け取れるので iframe 化自体は API 上可能だが、
  親が `contentDocument` に触る必要があるため opaque オリジンとは**原理的に両立しない**
  （ブラウザ側の制約）。採る案は未決。
