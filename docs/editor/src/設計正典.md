...
- **保存内容に GrapesJS 由来の揮発物を載せない**: 幾何（幅・余白）は inline `style` 属性に
  ...
  HTML 側に出ないため必ず死んだセレクタになる） — `getCss()` は component 木を歩いて「明示属性に
  無い id」を特定し、該当ルールだけを剥がす（本物の id セレクタをパターンだけで巻き込まない）。
  既定 `forceClass:true` は component 生成時に inline style が非空だと自動生成クラスへ丸ごと
