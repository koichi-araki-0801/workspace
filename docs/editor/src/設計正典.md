...
- **パーツコメントは 1 段の入れ子スレッドで、版インスタンスごとに独立する**: コメントは
  ...
  (open / resolved)・`replyTo`(親投稿 id。null なら親)を持ち、返信は同じパーツの親投稿にだけ
  付く(返信への返信は不可)。種別は持たない(コメントは「メモ」1 種類。旧データ・旧クライアントの
  `kind` は読み取りで捨てる)。状態の切替は親にだけ許し、
  ...
  2 属性を持たない投稿は読み取り時に既定値(open / null)を補って読む(ID は
  `legacy:<pathKey>` 固定)。
...
- **「未確定」は保存内容が確定版と違うときだけ**（2026-09-10）: `dirty` は `component:update`
  の発火ではなく、保存内容（`getBodyHtml()` + `getCss()`）が確定版の正規形と違うかで決める。
  正規形は `loadForEdit` 直後に canvas から取る（GrapesJS 自身の直列化どうしでしか文字列比較が
  成り立たない。draft があるときは確定版を先に読み込んで測ってから draft で入れ替える）。
  変更のたびに 300ms で突き合わせ、同じ内容へ戻れば「変更なし」へ下ろし draft を消す
  （`useTemplateEditor.settleIfClean`。autosave の予約より先に走らせ、同一内容の保存を取り消す）。
  この判定が成り立つための 3 つの不変則:
  - `getBodyHtml()` は**明示属性に無い id を出さない**。GrapesJS は選択したパーツに StyleManager の
    id セレクタを作り、以後 `getHtml()` が自動 id（`ccid`）を属性として出力する。混入すると
    再読込で確定版と構造キー（`partKey` / 赤入れ）が一致せず、編集していない箇所が赤入れになり、
    コメントの宛先が「削除済みパーツ」になる（実測）。
  - `getCss()` は GrapesJS の `protectedCss`（`* { box-sizing }` `body { margin }`）を**外す**
    （`avoidProtected`）。載せると次の `load` で規則として取り込まれ、Undo・往復のたびに積み増す。
  - `component:update` のうち保存内容に現れない prop（`status` / `open` / lock state の
    `editable` 等 = `grapesEvents.SAVE_NEUTRAL_PROPS`）だけの発火は dirty/autosave へ流さない。
    `open` は Layers がクリック選択のたびに祖先へ立てるため、濾さないと「選択しただけで未確定 +
    無編集 draft」になる。
  機械検証は `e2e/canvas.spec.ts`（選択のみ / 往復後の赤入れとコメント / Undo で変更なし）と
  `web/test/grapesEvents.test.ts`。
- **起動時のズームは 100%**（2026-09-10）: 編集キャンバスは画面に合わせず A4 実寸で開き、
  window・ペインの resize でも倍率を据え置く（追随させるのは overlay と縦配置だけ）。画面への
  フィットは Ctrl+0 / % ボタンの手動操作のみ（`useZoomFit.fitToView`）。
...
- **編集セッションはブラウザタブの寿命**（2026-09）: タブ遷移・プレビュー往復・リロードでは
  ...
  下書きを一斉に失わないため）。
  画面の UI 状態（編集許可 / 赤入れ表示 / 右ペインのタブ / 倍率 / ページ表示 / 選択パーツ）も
  同じセッションの一部で、プレビュー往復で戻す（`stores/editorSession.ts` の `ui`。
  `useTemplateEditor` と `EditorView` が変わるたびに写し、マウント時にそこから始める）。
  Undo と違い永続ミラーには載せない — リロードでは既定（100% / 1 ページ表示 / 未選択）から開く。
  機械検証は `e2e/canvas.spec.ts`「プレビュー往復で…残る」。
