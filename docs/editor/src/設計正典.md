...
4. CI 集約は `pnpm ci`（`check:comments → check:claude-hooks → check:ci → test:scripts →
   ...
   コミットする。rest e2e(`pnpm run e2e:rest`)は `E2E_REST=1` を呼び出し元シェルで設定し、sproc は
   in-memory フェイク(`server/test/fakes/sprocFake.ts`)を使う。フルの `ci` は `pie-chart:batch` →
   `pie-chart:batch:diff` で `pie-chart/out/_baseline` を要求するため、clone 直後はまずルート
   `README.md` の「フル `ci` の前提」節に従って基準を 1 回作る。
