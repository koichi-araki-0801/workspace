...
| 8 | 2. 用語置換 | ヘッダのみに適用 | `#chk-headers` | only_headers フラグ |
| 9 | 2. 用語置換 | クリック取り込みで折返しを連結 | `#chk-suggest-join` | suggest_join フラグ（既定 OFF。ON で連結取り込み・連結由来を記録） |
| 10 | 2. 用語置換 | このページのみ再適用 | `#btn-reapply-page` | 主ボタン（既定）。表示中ページへ辞書適用 |
| 11 | 2. 用語置換 | 全ファイル再適用 | `#btn-reapply` | 明示選択で辞書変更を全PDFへ |
| 12 | 2. 用語置換 | 辞書 入出力 | `#btn-dict-export / import` | JSON 入出力（連結由来 joined を保持） |
| 13 | 3. 削除・枠線 | ツール | `選択 / 範囲削除 / 枠線` | 編集モード切替 |
| 14 | 3. 削除・枠線 | 枠線色 | `#border-color` | カラーピッカー |
| 15 | 3. 削除・枠線 | 枠線幅 | `#border-width` | 0.5〜20 pt |
| 16 | 3. 削除・枠線 | 削除 | `#btn-deletesel` | 選択要素削除 |
| 17 | 4. 書き出し | 書き出し範囲 | `radio` | 全/現在/スキップ除く/ページ指定 |
| 18 | 4. 書き出し | SVGに書き出す | `button` | ファイル名は <元名>_pN.svg |
| 19 | トップバー | Undo/Redo | `Ctrl+Z / Ctrl+Y` | 操作の取消/やり直し |
...
| 4 | 入力 | only_headers | `bool` | ヘッダのみ置換 vs 全文 |
| 5 | 入力 | suggest_join | `bool（既定 false）` | クリック取り込みで折返し 2 行を連結するか |
| 6 | 入力 | 枠線色/幅 | `hex / 0.5〜20pt` | 枠線追加 |
| 7 | 入力 | 書き出し範囲 | `全/現在/指定` | ページ選別 |
| 8 | 出力 | SVGファイル | `決定的SVG` | 実<text>保持・使用グリフのみWOFF2埋込 |
| 9 | 出力 | PNG背景 | `ラスタ（SCAN_RENDER_SCALE=2.0）` | スキャンページ背景 |
| 10 | 設定 | config.py | `frozen exe / ソース共通` | exe 隣の data/ に辞書・ログ・作業領域 |
...
| 12 | RPC | `dictExport / dictImport` | 辞書JSONのファイル書き出し / 読み込み |
| 13 | RPC | `dictJson / dictImportJson` | 辞書JSONの文字列受け渡し |
| 14 | RPC | `setOnlyHeaders / setSuggestJoin` | ヘッダのみ適用 / クリック取り込み連結フラグ更新 |
| 15 | RPC | `applyDelete / deleteRegion / addBorder` | 削除 / 範囲削除 / 枠線（Undoへpush） |
| 16 | RPC | `undo / redo` | 操作の取消 / やり直し |
| 17 | RPC | `exportSvg` | 範囲指定で SVG 書き出し |
| 18 | RPC | `removeFile` | 選択ファイルを一覧から除去 |
...
| 5 | `test_wrap_header.py` | 折返しヘッダの畳み込み（複数行→1行。連結は joined エントリのみ） | ヘッダが1行化 | 未 |
