...
rev:
  - 1.0 | 2026-07-23 | 初版
  - 1.1 | 2026-08-15 | 辞書適用の原則を話題ごとに分割、接続の資源上限の重複記述を統合、用語の初出説明を補足
---
...
| `src/web/` | ローカル HTTP サーバ（静的配信 + `/rpc` 24 メソッド + `/upload` `/quit` `/ping`）・コマンド・Undo |
...
- **辞書適用は明示操作のみ・既定は表示中ページ**: アップロード時の自動適用は行わない。適用は
  再適用ボタン（Undo 可能な Command マクロ経路）に一本化する。主ボタンは「このページのみ再適用」で、
  全ファイルへの適用は明示選択。
- **適用対象はヘッダ・本文を問わず全文**: 「ヘッダのみ適用」トグルは廃止済み。ヘッダ判定は
  確認一覧の表示ラベルのみに使う。
- **折返し連結照合は連結由来（`joined=True`）エントリのみ**に一致させる: クリック取り込みの連結
  （チェック ON 時のみ）で登録した語だけが 2 行畳み込みの対象で、手入力・旧形式 JSON の語は
  単独行照合のみ（`DictionaryStore.lookup_wrap`）。
- **連結置換後のテキストは下揃え + 元の行揃えを踏襲**: 合成領域内で縦は最終行のベースラインへ
  下揃えし、横は元の折返し行の揃え（左/中央/右）を検出して踏襲する。全幅引き伸ばしはせず、
  幅超過時のみ `textLength` で圧縮する。
...
- **`/rpc` が返す pageSvg は生成点（exporter）で守る**: 属性は `_attr` / `quoteattr` 経由でのみ
  組み立て、f-string 等で `="{...}"` 形に生の値を差し込まない。web 側が受け取った pageSvg を
  そのまま innerHTML で描画してよいのは、この生成点の規律が担保しているからである。規律が
  崩れていないことの機械検証は `test_export_escaping.py` の `src/export/` 配下ソース走査ガード
  が行う。
...
- **ヘッダ検査は非ブラウザには効かない。非安全メソッドはセッショントークンで認可する**
  ...
  を発行し、`parse_request()` の G6（トークン検査の段）で `hmac.compare_digest` の
  **定時間・完全一致**を要求する（検査を足す場所はここ 1 箇所だけ。ハンドラやディスパッチャには書かない）。
...
- **トークンを配信物へ埋めない**（却下済み）。`GET /` は安全メソッドで誰でも撃てるため、
  ...
  同じ理由で **`startup.log` にトークンを書かない**（ログはディスクに残り、可搬運用では配布物の隣に置かれる）。
...
- **接続の資源上限は「量」と「本数」の両方で持つ**（2026-08-07 追加）。本文サイズ上限
  ...
  `parse_request()` のガードはリクエスト行とヘッダが届いて初めて走るので、**何も送らない
  接続には届かない**。`Handler.timeout`（= `REQUEST_TIMEOUT`。無通信 10 秒でソケットを閉じる）は
  **ガードの手前**を閉じる唯一の手段で、`BoundedThreadingHTTPServer`（`MAX_CONNECTIONS` =
  同時接続 32）は超過分をキューに積まず即切断する。この 2 つは対で要る。
  拒否後の `_linger_close` の期限は per-recv でなく**接続単位**で持つ（per-recv だと
  「0.4 秒ごとに 1 バイト」で 1 MiB 分スレッドを握られる）。同じ理由で**要求の読み取り本体も
  接続単位の絶対期限で縛る**（`MAX_REQUEST_SECONDS` / `_DeadlineSocketIO`。各 recv の手前で
  残り時間へタイムアウトを張り直す）。`Handler.timeout` は recv ごとに再武装される無通信上限
  なので、改行を送らず per-recv 未満の間隔で 1 バイトずつ送れば `readline` を無限に引き延ばせた。
  拒否経路だけでなく**受理側の読み取り**にも同じ「期限は単位ごと」を効かせる。
...
- **辞書ファイルは外部由来**（メール・共有フォルダで配られる）。要素 1 個の型崩れで
  ...
  画面に出ないため、そのまま**エラーも出ない起動不能**になる。壊れた要素は
  捨てて起動し、件数を通知する。
- **可変状態はプログラムフォルダの外**（2026-08-07 追加）。onedir 配布の exe
