...
rev: 2.0 | 2026-07-04 | 章立ての階層化・アーキテクチャ図の追加・層別詳解への拡充
rev: 2.1 | 2026-07-05 | 表紙・改訂履歴の新設、経緯記述の整理と文章の推敲
rev: 2.2 | 2026-07-23 | 読み進め方の新設、図 4 種（アクティビティ・状態遷移・シーケンス・クラス）の追加
rev: 2.3 | 2026-07-24 | 課題・仮説・MVP(1.2)、パフォーマンス(13 章)、開発・運用フロー(16 章)、リンク集(17 章)の新設、依存関係章(15 章)への拡張と章番号再編
audience: spec
---

本書は PdfToSvg の実装構造・依存・ライフサイクルを、Python にまだ慣れていない読者でも実装を追えるところまで掘り下げて記述する。対象リビジョンは最新版。本文中のコード抜粋はすべて実ソースからの引用で、引用元をパスで示す。

**読み進め方**: 全体像だけ知りたいときは 1 章と 3 章（アーキテクチャ図・変換パイプライン図）まで
で足りる。データ構造から追うなら 4 章（クラス図）→ 5 章（抽出）→ 6 章（書き出し）の順、
編集操作の仕組みを追うなら 8 章（RPC シーケンス図・コマンド / Undo）→ 9 章（UI）の順が近道。
配布・起動まわりのトラブルは 11〜12 章、用語に迷ったら 2 章の用語解説へ戻るとよい。
性能の設計判断は 13 章、テストは 14 章、依存とライセンスは 15 章、開発・運用の取り決めは
16 章、関連資料へのリンク集は 17 章にまとめる。
...
Edge をアプリモードで開く方式のため QtWebEngine ランタイムを同梱せずに済み、配布物は ~50MB に収まる。

## 1.2 課題・仮説・MVP

**課題**: 帳票 PDF の用語統一（表ヘッダの言い換え等）は、これまで PDF を直接編集するか元原稿へ差し戻すかの二択だった。PDF 直編集は再編集不能な一点物を量産し、置換漏れの確認も目視頼み。元原稿差し戻しは持ち主（別部署・外部）との往復が発生し、締切に間に合わない。

**仮説**: 「PDF を本物の `<text>` を持つ SVG へ変換すれば、テキストとして検索・置換・再編集できる成果物になる。置換を NFKC 正規化の永続辞書で自動化すれば、全角/半角の表記ゆれも吸収して事務担当者が単独で完結できる」。成果物を PDF でなく SVG としたのは、編集可能性を最優先し、閲覧・印刷は SVG でも足りるためである。

**MVP と段階**: 最初のリリース（Phase 1）は PySide6/QtWebEngine シェルで変換・辞書置換・削除・書き出しの中核を成立させた。Phase 2 で UI シェルを OS 標準 Edge のアプリモード + ローカル HTTP サーバへ置き換え、Qt ランタイム同梱を廃して配布物を ~50MB へ縮小した（シェルは替わったが「model is truth」のモデル・エクスポータは Phase 1 から不変）。機能追加より先に、VDI を含む社内全端末で確実に起動することへ投資している（12 章）。
...
PDF 入力から SVG 出力までの処理の流れ（アクティビティ図）。辞書置換は `/upload` 直後の自動適用
（`auto_apply`）が最初の適用点で、以降の再適用・手動編集は Web UI から Undo 可能なコマンドとして
積まれる。未確認ガード（9 章）は書き出しの手前に立つ:
...
flowchart TD
  ...
  Z --> DOC
  DOC --> DICT["辞書 自動適用<br/>dictionary/apply.auto_apply"]
...
で直列化され、置換 N 件はマクロ 1 束（= 1 Undo）として積まれる（8 章）:
...
> [!WARN] `pdf_engine.py` は **AGPL ライセンスの PyMuPDF** に依存する唯一のモジュール（15 章）。差し替え候補（pypdfium2）を見据えて、PyMuPDF への依存はこのファイルに閉じている。
...
そもそも「使用グリフのみ」のサブセット化自体が最大の軽量化で、和文フォント全体（数 MB）を埋め込まずに済む。出力サイズの上限はテストで機械監視する（14 章）。

## 13.2 スキャンページの扱い（5.2 節）
...
QtWebEngine ランタイム非同梱（~50MB 配布）、外部通信ゼロ、`data/tmp/<pid>` のローカル作業領域（10.1 節）は、いずれも起動時間と配布サイズを直接短くしている。性能改善の第一手は「重い依存を足さない」であり、新依存の追加はこの観点の審査を先に通すこと。
...
**技術要件**: Windows 10/11 + システム標準 Edge が唯一の環境前提（1.1 節）。外部サービス・外部通信は無く、管理者権限も不要。ネットワークは 127.0.0.1 のローカル HTTP のみで、プロキシ・証明書などの社内ネットワーク設定に依存しない。同梱フォント（BIZ UDPゴシック / Noto Serif JP）は SIL OFL 1.1 で再配布可能なものだけを選んでいる（10.2 節）。
...
- コミット条件は `python -m pytest` 全通過（14.1 節）。モデル・エクスポータの変更は書き出し出力の互換（`annotate=False` 完全一致）を守る。
- PyMuPDF（AGPL）への依存は `engine/pdf_engine.py` に閉じたまま保つ。他モジュールから `fitz` を import しない — 将来の pypdfium2 差し替え（15 章）の前提条件である。
- Edge 起動フラグ・ファイル入出力を触る前に 12 章（VDI 制約）を必ず読む。非標準フラグ追加とネイティブピッカーは禁止事項。

## 16.2 ビルド・配布

実手順は別冊「PdfToSvg 配布・運用手順書」を正典とする。要点のみ: `scripts\build.bat` が隔離 venv（`.venv-build`）で PyInstaller onedir を生成し（11.1 節）、配布物に Qt DLL が無いことをチェックリストで確認する。

## 16.3 障害対応の逆引き

| 症状 | 見る場所・対処 |
|---|---|
| アプリ窓が開かない・すぐ落ちる | `data/logs/startup.log`（Python 側）→ `data/logs/edge.log`（Edge 側）の順に切り分け（12 章） |
| VDI でだけクラッシュする | 起動フラグの非標準化が疑い筆頭。既定 Edge と同じ構成へ戻す（12 章） |
| 置換されない語がある | 辞書の `enabled`、NFKC 正規化キーの一致、折返しヘッダの連結条件（7.3 節）を順に確認 |
| 出力 SVG が異常に大きい | フォントサブセット失敗（原本フォント不在等）の兆候。13.1 節と `test_pipeline.py` のサイズ上限を参照 |
| 終了してもプロセスが残る | `/quit` ビーコン不達時は idle watchdog（60 秒）で自然終了する。それでも残る場合は `startup.log` で watchdog 動作を確認（11.2 節） |
...
- **pypdfium2 への差し替え**: AGPL（PyMuPDF）解消の本命。`engine/pdf_engine.py` の隔離が済んでいるため、差し替えはこのファイルの再実装で完結する（15 章）。
- **パスベース辞書入出力の整理**: `dictExport`/`dictImport` の旧経路は現 UI から未使用であり、整理候補（8.2 節）。
