...
`WebSession` がサーバ側の全状態を持つ: `store`（辞書）・`undo`（Undo スタック）・`docs`（開いている Document のリスト）・`only_headers`（ヘッダのみ適用フラグ）・`suggest_join`（クリック取り込みで折返しを連結するか。既定 False）。

`HANDLERS` には**全 24 メソッド**が登録されている:
...
| 辞書適用 | `setOnlyHeaders`, `setSuggestJoin`, `reapplyDict`, `reapplyDictPage` |
