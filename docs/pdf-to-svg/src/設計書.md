...
1. `_find_edge()` がレジストリ（App Paths）→既知パスの順に msedge.exe を探索。
2. 専用 user-data-dir（temp）を作り `--app=URL` で独立プロセスとして起動。
3. ローカル HTTP サーバを daemon スレッドで起動（ポートは 0 指定で OS 自動割当）。
4. 終了契機は窓を閉じた時の `/quit` ビーコン、または `/ping` 途絶を見張る idle watchdog（60 秒）。
5. 終了時にサーバを畳み、Edge を terminate、temp プロファイルを削除。
