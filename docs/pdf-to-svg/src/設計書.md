...
| `data/tmp/<pid>/` | 実行時の作業領域（Edge プロファイル / PDF・JSON 一時）。**終了時に丸ごと削除** |
...
1. 起動時に実行専用の作業領域 `data/tmp/<pid>` を作り、`tempfile` の既定先をそこへ付け替える（実行時に書く全 temp を exe 隣の `data/` へ寄せ、C:/`%TEMP%` に触れない）。
2. `_find_edge()` がレジストリ（App Paths）→既知パスの順に msedge.exe を探索。
3. user-data-dir（`data/tmp/<pid>/edge-profile`）を使い `--app=URL` で独立プロセスとして起動。子プロセスの `TEMP`/`TMP` も `data/tmp/<pid>` に向ける。VDI 対策フラグ（`--disable-gpu` 等）と Edge ログ（`--enable-logging --log-file=data/logs/edge.log`）を付ける。
4. ローカル HTTP サーバを daemon スレッドで起動（ポートは 0 指定で OS 自動割当）。
5. 終了契機は窓を閉じた時の `/quit` ビーコン、または `/ping` 途絶を見張る idle watchdog（60 秒）。
6. 終了時にサーバを畳み、Edge を terminate、`data/tmp/<pid>` を丸ごと削除（プロファイル・一時を一括清掃）。
...
> [!INFO] **VDI/リモートデスクトップ対応**: これらの環境は実 GPU が無く Chromium(Edge) の GPU/コンポジタがクラッシュしてアプリ窓が「一瞬出て消える」ため、`--disable-gpu` 等でソフトウェア描画へ固定する。さらに C ドライブ/`%TEMP%` がアクセス不可な VDI でも動くよう、実行時 temp と Edge プロファイルを exe 隣の `data/tmp/<pid>` に集約し、子 Edge の `TEMP`/`TMP` もそこへ向ける。なお落ちる場合は `data/logs/startup.log`（Python 側）と `data/logs/edge.log`（Edge 側）で要因を切り分ける。
