...
なお落ちる場合はまず `data/logs/startup.log`（Python 側）を見る。Edge 側の詳細が要るときだけ `PDFTOSVG_EDGE_LOG=1` を立てて再現させ、`%LOCALAPPDATA%\PdfToSvg\logs\edge.log` を見る（既定では出力されない。既定プロファイルで開くため、常時出力すると利用者の Edge セッション全体の記録が溜まり、`--app=` の URL に載せたセッショントークンごとディスクに残りうる）。
...
| 症状 | 見る場所・対処 |
|---|---|
| アプリ窓が開かない・すぐ落ちる | `data/logs/startup.log`（Python 側）→ `PDFTOSVG_EDGE_LOG=1` で再現させて `%LOCALAPPDATA%\PdfToSvg\logs\edge.log`（Edge 側）の順に切り分け（11 章） |
