...
// =============================================================================
// affected ランナーの実害は「走ったつもりで一段も走っていない」形で出る (pdf-to-svg /
...
test('pdf-to-svg は pytest・vitest・e2e の 3 段', () => {
  assert.deepEqual(planForChanges(['pdf-to-svg/src/web/server.py']), [
    ...SHARED_GATES,
    'test:pdf-to-svg',
    'test:pdf-to-svg:js',
    'e2e:pdf-to-svg',
...
test('graph-editor は typecheck・vitest・pytest・e2e の 4 段', () => {
  assert.deepEqual(planForChanges(['graph-editor/resources/web/js/editor.js']), [
    ...SHARED_GATES,
    'typecheck:graph-editor',
    'test:graph-editor',
    'test:graph-editor:py',
    'e2e:graph-editor',
  ]);
});
