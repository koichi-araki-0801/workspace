...
.PARAMETER SkipRegen
  重量物の再生成（pnpm install / npm pack / playwright install）を省略し、
...
if (-not $SkipRegen) {
  ...
  Write-Host "[2/3] pnpm 本体 tarball を取得（npm pack pnpm@$pnpmVersion）..."
  Get-ChildItem $RepoRoot -Filter 'pnpm-*.tgz' -File -ErrorAction SilentlyContinue | Remove-Item -Force -ErrorAction SilentlyContinue
  & npm pack "pnpm@$pnpmVersion" --pack-destination $RepoRoot
  if ($LASTEXITCODE -ne 0) { Write-Error '[error] npm pack pnpm に失敗しました。'; exit 1 }
  $packed = Get-ChildItem $RepoRoot -Filter 'pnpm-*.tgz' -File | Sort-Object LastWriteTime | Select-Object -Last 1
  if (-not $packed) { Write-Error '[error] pnpm-*.tgz が生成されませんでした。'; exit 1 }
  Move-Item $packed.FullName $PnpmTgz -Force
...
if (-not $releaseExists) {
  ...
  & gh release delete-asset $Tag $oldAsset --yes *> $null
}
