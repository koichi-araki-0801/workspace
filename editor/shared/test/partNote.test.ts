...
describe('PartNoteEntry のコメント属性', () => {
  ...
  it('status / replyTo を持つ形を受理する', () => {
    const res = PartNoteEntry.safeParse({ ...base, status: 'resolved', replyTo: 'p1' });
  ...
  // コメントは「メモ」1 種類だけ。旧データ・旧クライアントの `kind` は受理しても出力に残さない。
  it('旧形式の kind は捨てる(コメントはメモのみ)', () => {
    const res = PartNoteEntry.parse({ ...base, status: 'open', replyTo: null, kind: 'question' });
...
describe('AddNoteRequest の返信', () => {
  ...
  it('replyTo を受理し、旧クライアントの kind は捨てる', () => {
