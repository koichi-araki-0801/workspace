...
SQL Server 2012 の `usrap.ug01`（既存 DB / 既存スキーマ）に、`Rep1_運報自動化_Editor_` 接頭辞でテーブルとストアドを作成する。本文(HTML/CSS)はファイル保存、DB は台帳・履歴・監査などメタの索引のみ。

## 構成
```
ddl/    01_テーブル.sql 02_索引.sql 03_制約.sql   … 7 テーブル（冪等）
sproc/  template/history/user/session/part/sample/audit.sql
