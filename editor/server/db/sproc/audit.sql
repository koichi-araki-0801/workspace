 ...
 *  @操作: 登録(best-effort, logger.audit から二重化) / 検索
 *  JSON はテキスト保管。検索は最新 500 件(イベントで任意絞込)。
 * ==========================================================================*/
...
BEGIN
  ...
  BEGIN
    SELECT TOP (500)
           [監査ID], [イベント], [結果], [実行者], [IP],
           [リソースJSON], [詳細JSON], [エラー], [発生日時]
      FROM [ug01].[Rep1_運報自動化_Editor_監査ログ]
      WHERE (@イベント IS NULL OR [イベント] = @イベント)
      ORDER BY [発生日時] DESC;
    RETURN;
  END
