...
/* パーツカタログ.次回反映既定 ∈ {反映, 非反映} (NULL = 未判断 = 反映しない) */
IF OBJECT_ID(N'[ug01].[CK_パーツ_次回反映既定]', N'C') IS NULL
  ALTER TABLE [ug01].[Rep1_運報自動化_Editor_パーツカタログ]
    ADD CONSTRAINT [CK_パーツ_次回反映既定]
      CHECK ([次回反映既定] IS NULL
             OR [次回反映既定] IN (N'反映', N'非反映'));
GO
