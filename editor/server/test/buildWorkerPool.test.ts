...
describe('BuildWorkerPool', () => {
  ...
  // ── 行列の上限と資源確保の順序 ──
  // 待機列が無制限だと、1 ジョブが最大 timeoutMs(既定 120s)掛かるあいだ到着した
  // リクエストを全部並べる。さらに呼び出し側が**並ぶ前に** temp ディレクトリ・配信ルートへの
  // 資産配置・egress の許可ポート(1 ビルド 4 個)を確保すると、行列の長さがそのまま
  // 資源消費になる。ここでは「上限が効くこと」と「確保が枠取得の後であること」を主張する。
