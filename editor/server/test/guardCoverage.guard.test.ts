...
// =============================================================================
// ガードの迂回は、個別の穴というより**同じ 1 つの型**で生まれる: 新しい守りを入れたとき、
// 旧い守りが覆っていた範囲と新しい守りの射程の差分を数えないと、差分の入口が素通りになる。
//
// 個別の穴を塞ぐだけでは同じ型がまた生えるので、「このガードが覆う入口はこれで全部」という
// **列挙そのもの**を機械検証する。列挙の外に入口が増えたらここが落ちる。
//
...
describe('nunjucks が外部の値へ触る経路は全部包む', () => {
  ...
  // の 4 経路。前 2 者だけを包むとフィルタ解決が素通りになる。テスト解決はフィルタ解決と
  // 同型で、`getTest` も継承プロパティを引く。
