...
// =============================================================================
// `POST /auth/init-password` がセッションも所有証明も要求しないと、`{"username":"admin",...}`
// を投げるだけで任意アカウントを乗っ取れる。ここで守るのは前段(セッション必須・宛先は
...
describe('POST /auth/init-password の施錠', () => {
  ...
  beforeAll(async () => {
    ...
    await app.register(authRoutes, { prefix: '/api', deps });
    await app.ready();
  });
