...
// =============================================================================
// `viewer` は閲覧のみのロールだが、台帳の上にあるだけでは下書きの上書き・削除、
// メモ書き、履歴追記、承認申請の投入まで全部通れる。ここで主張するのは
...
vi.mock('../src/db/sproc.js', () => ({
  ...
  // `auth/session.ts` の既定ストアが読む実行面。ここでは誰も呼ばないが、export が
  // 欠けると module 解決の時点で落ちる。
  realSproc: { callSproc: vi.fn(async () => []) },
  firstRow: () => undefined,
  rows: () => [],
