/**
 * ローカル・フィクスチャ向けのファンド業務ルール。
 *
 * 本番（REST/SQL Server）では台帳・sproc が同等の判定を担うが、ローカル動作では
 * フィクスチャ駆動で表現する。
 */

/**
 * シリーズファンド＝「コア投資戦略ファンド（コアラップ）」系のファンドコード集合。
...
/**
 * 償還ファンド・モック: 作成済みテンプレHTMLの特定パーツを償還用パーツへ置換する。
 *
...
export function applyRedemptionMock(html: string): string {
  ...
  // 2) 償還サマリー帯を本文先頭（<body> 直後）へ挿入。レイアウトに依存しないよう
  //    特定セクションの置換ではなく <body> 起点で挿入する。値は sample から差し込み。
