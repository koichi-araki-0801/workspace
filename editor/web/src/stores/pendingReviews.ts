...
export const usePendingReviewsStore = defineStore('pendingReviews', () => {
  ...
  /**
   * 承認待ち一覧を取り直す。バッジ表示はベストエフォート(主導線はタブに残る)のため、
   * 取得失敗はエラー表示にせず空へ倒す — 前回値の残置だと、ログインし直した別ユーザーへ
   * 前ユーザーの件数を見せてしまう。
   */
  async function refresh(): Promise<void> {
    const res = await reviews.listReviews({ status: 'pending' });
    items.value = isOk(res) ? res.value : [];
  }
