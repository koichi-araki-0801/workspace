...
describe('CommentPanel', () => {
  ...
  it('種別の入力欄・絞り込みは出さず、行のバッジは状態(未対応 / 解決済み)を示す', async () => {
    ...
    const coverRow = rows.find((r) => r.text().includes('表紙の日付'));
    const summaryRow = rows.find((r) => r.text().includes('要約の数値'));
    expect(coverRow?.text()).toContain('未対応');
    expect(summaryRow?.text()).toContain('解決済み');
