// =============================================================================
// verify/oracle_sync.ts — verify オラクル (`svg.ts`) の複製定数/文字幅分類の drift ガード
// =============================================================================
// `verify/svg.ts` は検証 *ロジック* を本体から import せず独立オラクルとして複製するが、
// そこで複製する *数値定数* と *文字幅分類* だけは本体 (`config.ts` の既定値 /
// `geometry.ts` の `visualCharEm`) と一致していなければならない。ここに定数と照合関数
// `assertOracleSync` を集約し、`verify/svg.ts`(CLI)は import して起動時に呼ぶ。テスト
// (`test/oracle_sync.test.ts`) からも同関数を直接叩けるよう、乖離時は `process.exit` ではなく
// `Error` を throw する (CLI 側で捕捉して exit(1) する)。SVG 出力経路 (`cli.ts` / render) には無関係。
...
// assertOracleSync() が本体 (`config.ts` の既定値 / `geometry.ts` の `visualCharEm` の判定域) と一致するか
...
/**
 * verify のローカル複製定数/文字幅分類が本体 (`config.ts` / `geometry.ts` の `visualCharEm`) と
...
export function assertOracleSync(): void {
  ...
  // 効いているかを代表コードポイントで検査する。テーブル未生成/未適用なら基底 heuristic の
