...
const OUT_DIR = path.join(ROOT, 'src', 'glyph_advance');
...
// ${rel} (unitsPerEm=${unitsPerEm}) の実 glyph advance(em)。収録数: ${entries.length}
// visualCharEm の heuristic 予測 (全角1.0/半角0.5) と異なる codepoint のみ収録。
// フォント差し替え時は \`npm run gen:widths\` で再生成する。
export const GLYPH_ADVANCE_${weight}: ReadonlyMap<number, number> = new Map<number, number>([
${lines.join('\n')}
]);
`;
  fs.writeFileSync(path.join(OUT_DIR, `weight_${weight}.ts`), out, 'utf-8');
  summary.push(`${weight}:${entries.length}`);
}
console.log(`wrote ${OUT_DIR}/weight_{400,700}.ts: heuristic-diff entries (${summary.join(', ')})`);
