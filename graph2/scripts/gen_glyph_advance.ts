// =============================================================================
// gen_glyph_advance.ts — 埋め込みフォント (fonts/BIZUDPGothic-Bold.woff2) の実 glyph advance
// 幅をパースし、非 1.0 em の codepoint→advance(em) 例外テーブルを src/glyph_advance.ts へ出力する。
// -----------------------------------------------------------------------------
...
// かなも載り、'%' のような全角幅 ASCII も載る)。フォントを差し替えたら本スクリプトを再実行する。
...
const FONT_PATH = path.join(ROOT, "fonts", "BIZUDPGothic-Bold.woff2");
...
// 埋め込みフォント fonts/BIZUDPGothic-Bold.woff2 (unitsPerEm=${unitsPerEm}) の実 glyph advance(em)。
...
console.log(`wrote ${OUT_PATH}: ${entries.length} heuristic-diff entries (unitsPerEm=${unitsPerEm})`);
