...
if ($bundleChanged) {
  ...
  if (-not $SkipRegen) {
    ...
    try {
      ...
      foreach ($m in @('editor\shared', 'editor\server', 'editor\web', 'pie-chart')) {
    ...
    # docs\_build\build_all.bat は、この python-wheelhouse から --no-index でオフライン
    # install する。リポジトリ追跡中の全 requirements.txt（`Get-OfflineRequirementsFiles`）の
    # 和を 1 ディレクトリへ収集する。
    $pyExe = $null
    ...
    # なお `--no-deps` / `--require-hashes` は入れていない: requirements.txt は版を == 固定
    # 済みだがハッシュは固定していないため、入れると wheelhouse が推移依存を欠いて配布先の
    # オフライン install が壊れる。requirements の hash 化は別作業（積み残し）。
