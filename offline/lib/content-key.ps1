...
function Get-OfflineRequirementsFilesViaFileSystem {
  ...
  # -Filter のワイルドカードは 8.3 短名の意味論を踏むため、後段の EndsWith で判定する。
  $found = Get-ChildItem -LiteralPath $repoFull -Recurse -File -Force -ErrorAction SilentlyContinue |
    Where-Object { $_.Name.EndsWith('requirements.txt') } |
    Where-Object {
